...
const TopicItem = styledTS<{ isActive: boolean }>(styled.li)`
  position: relative;
...
const TopicTitle = styledTS<{ isChild?: boolean }>(styled.span)`
  ${props => props.isChild && `padding-left: 20px;`}
`;

export { Topics, TopicItem, ActionButtons, TopicTitle };
