...
class Filter extends React.Component<Props, State> {
  ...
  renderConjunction = () => {
    const { index, segmentKey, changeConjunction, conjunction } = this.props;

    if (index === 0) {
      return <></>;
    }

    if (changeConjunction) {
      const onClickAnd = () => {
        changeConjunction(segmentKey, 'and');
      };

      const onClickOr = () => {
        changeConjunction(segmentKey, 'or');
      };
      ...
      return (
        <ConjunctionButtons>
          <Button.Group hasGap={false}>
            ...
            <Button btnStyle={btnSyleOr} onClick={onClickOr}>
              {__('Or')}
            </Button>
          </Button.Group>
        </ConjunctionButtons>
