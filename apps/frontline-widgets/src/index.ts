...
import { listenForCommonRequests } from './lib/utils';
...
// SPA support: some host sites re-inject this <script> tag on every
// client-side route change or component remount instead of loading it once
// — e.g. a Next.js `pages/[slug]` app rendering a different
// `window.erxesSettings.messenger.integrationId` per dynamic page. Wrapping
// the whole module in this IIFE lets us bail out early (via `return`) when
// the widget is already mounted, instead of duplicating containers, iframes,
// and window message listeners — but first re-deliver the (possibly
// updated) settings to the existing iframe so it can reconnect with the new
...
(function initErxesMessengerWidget() {
  ...
  if (existingContainer) {
    ...
    if (existingIframe?.contentWindow) {
      sendMessageToIframe(existingIframe.contentWindow);
    }
  ...
  window.addEventListener('message', async (event) => {
    ...
    // Answers the iframe's 'requestingBrowserInfo'/'setLocalStorageItem'
    // requests using the host page's own window.location (this script runs
    // in the host page, not inside the iframe).
    await listenForCommonRequests(event, messengerIframe);
  ...
  // ---------------------------------------------------------------------------
  // SPA route-change tracking — client-side routers (React Router, Vue
  // Router, Next.js, etc.) navigate via the History API without a full page
  // reload, so the messenger iframe would otherwise never learn about
  // subsequent page views after the initial connect. Patch pushState /
  // replaceState and listen for popstate / hashchange, then notify the
  // iframe so it can re-save browser info for accurate page-view tracking.
  // ---------------------------------------------------------------------------
  ...
  const notifyLocationChange = () => {
    ...
    if (!messengerIframe.contentWindow) return;
