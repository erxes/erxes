...
`;
  ...
  let v = '';
  let x = !1;
  const B = 'erxes-messenger-container';
  const W = 'erxes-messenger-iframe';
  const X = `
  ...
  const z = async () => {
    i = a.contentDocument || a?.contentWindow?.document;
    if (i) {
  ...
  const S = () => localStorage.getItem('erxes') || '{}';

  const I = (t, e) => {
    const n = window.Erxes || {};
    n[t] = e;
    window.Erxes = n;
  };
  ...
  const F = (t) => {
    const n = window.erxesSettings;
    const e = n == null ? void 0 : n.messenger;
  ...
  window.addEventListener('message', async (t) => {
    const { data: e } = t;
    const { isVisible: n, message: m, isSmallContainer: d } = e || {};
    if (e.fromErxes && e.source === 'fromMessenger') {
      ...
      if (y) {
        document.body.classList.toggle('widget-mobile', n);
      }
      if (m === 'messenger') {
        if (y && n) {
          P();
