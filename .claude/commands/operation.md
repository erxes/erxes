---
description: Operation Workflow — grant a customer-facing Operation feature wish end-to-end (7 phases with gates)
argument-hint: "<wish text>"
allowed-tools: Read, Write, Edit, Bash, Grep, Glob, TaskCreate, TaskUpdate, TaskList, TaskGet, Agent
---

# /operation — Operation Workflow orchestrator

You are entering the Operation Workflow. This is **not** a free-form coding session. You execute 7 phases in order. Each phase has a gate. **Do not skip a phase.**
...
1. Generate a wish ID: `YYYY-MM-DD-<slug>` where slug is a short kebab-case derived from the wish (e.g., `2026-05-22-task-urgency-badge`).
...
1. Read each file in `.agents/skills/operation/` (their "When to use" headers).
2. Decide: does this wish fit a skill, or is it NOVEL?
3. Append a `## Routing` section to `WISH.md`:
   ```markdown
   ## Routing
   **Skill:** `.agents/skills/operation/<skill>.md`
...
2. Run `.agents/evals/run.sh operation` (or `--backend-only`/`--frontend-only` per the commit's scope).
3. If exit 0:
   - `git add -A`
...
5. After all commits: re-run `evals/run.sh operation` with full default (no `--*-only` flags). Must exit 0.
...
1. Identify the relevant Playwright spec at `.agents/plugins/operation/tests/<file>.spec.ts` (most often `task.spec.ts`).
...
5. Run: `cd .agents && pnpm test plugins/operation/tests/<file>.spec.ts`
...
8. Run `.agents/evals/run.sh operation --include-e2e` for a final all-green confirmation.
