...
(function() {
  CKEDITOR.plugins.add('codemirror', {
    icons:
      'searchcode,autoformat,commentselectedrange,uncommentselectedrange,autocomplete', // %REMOVE_LINE_CORE%
    lang:
      'af,ar,bg,bn,bs,ca,cs,cy,da,de,el,en-au,en-ca,en-gb,en,eo,es,et,eu,fa,fi,fo,fr-ca,fr,gl,gu,he,hi,hr,hu,is,it,ja,ka,km,ko,ku,lt,lv,mk,mn,ms,nb,nl,no,pl,pt-br,pt,ro,ru,sk,sl,sr-latn,sr,sv,th,tr,ug,uk,vi,zh-cn,zh', // %REMOVE_LINE_CORE%
    version: '1.17.10',
    init: function(editor) {
      ...
      var requirePresent =
        'function' === typeof require && 'function' === typeof require.config;
      var pluginRequire;
      if (requirePresent) {
      ...
      if (
        ...
        CKEDITOR.dialog.add('sourcedialog', function(editor) {
          ...
          function loadCodeMirrorInline(editor, textarea, dialog) {
            ...
            window['codemirror_' + editor.id] = CodeMirror.fromTextArea(
              textarea,
              {
                mode:
                  config.mode === 'handlebars'
                ...
                extraKeys: {
                  'Ctrl-Q': function(codeMirror_Editor) {
                    if (config.enableCodeFolding) {
                      window['foldFunc_' + editor.id](
                        codeMirror_Editor,
                        codeMirror_Editor.getCursor().line
                      );
                    }
                  }
                },
            ...
            if (config.autoFormatOnStart) {
              if (config.useBeautifyOnStart) {
                ...
                window['codemirror_' + editor.id].setValue(
                  html_beautify(
                    source,
                    indent_size,
                    indent_char,
            ...
            window['codemirror_' + editor.id].setSize(
              holderWidth,
              holderHeight
            );
            ...
            if (config.lineNumbers && config.enableCodeFolding) {
              window['codemirror_' + editor.id].on(
                'gutterClick',
                window['foldFunc_' + editor.id]
              );
            }
            ...
            window['codemirror_' + editor.id].on('keypress', function(
              codeMirror_Editor,
              evt
            ) {
              if (config.enableCodeFormatting) {
                ...
                if (
                  ...
                  window['codemirror_' + editor.id].commentRange(
                    true,
                    range.from,
                    range.to
                  );
            ...
            if (
              editor.plugins.textselection &&
              textRange &&
              !editor.config.fullPage
            ) {
              var start, end;

              start = OffSetToLineChannel(
                window['codemirror_' + editor.id],
          ...
          return {
            ...
            onShow: function(event) {
              ...
              this.getContentElement('main', 'AutoComplete').setValue(
                config.autoCloseTags,
                true
              );
              ...
              if (config.autoLoadCodeMirror) {
                if (
                  !IsStyleSheetAlreadyLoaded(
                    rootPath + 'css/codemirror.min.css'
                  )
                ) {
                ...
                if (
                  ...
                  !IsStyleSheetAlreadyLoaded(
                    rootPath + 'theme/' + config.theme + '.css'
                  )
                ) {
                  CKEDITOR.document.appendStyleSheet(
                    rootPath + 'theme/' + config.theme + '.css'
                ...
                if (requirePresent) {
                  ...
                  if (typeof CodeMirror === 'undefined') {
                    CKEDITOR.scriptLoader.load(
                      rootPath + 'js/codemirror.min.js',
                      function() {
                        CKEDITOR.scriptLoader.load(
                          getCodeMirrorScripts(),
                          function() {
                            loadCodeMirrorInline(
                              editor,
                              textArea,
                              event.sender
                            );
            ...
            onOk: (function() {
              ...
              return function() {
                ...
                var newData = this.getValueOf('main', 'data').replace(
                  /\r/g,
                  ''
                );
                ...
                elements: [
                  {
                    ...
                    children: [
                      ...
                      },
                      {
                        ...
                        onClick: function() {
                          var range = {
                            from: window['codemirror_' + editor.id].getCursor(
                              true
                            ),
                            to: window['codemirror_' + editor.id].getCursor(
                              false
                            )
                          };
                          window['codemirror_' + editor.id].autoFormatRange(
                      ...
                      },
                      {
                        ...
                        onClick: function() {
                          ...
                          if (
                            window['codemirror_' + editor.id].config
                  ...
                  },
                  {
                    ...
                    onChange: function() {
                      window['codemirror_' + editor.id].setOption(
                        'autoCloseTags',
                        this.getValue()
                      );
                    }
      ...
      if (!sourcearea.commands.searchCode) {
        CKEDITOR.plugins.sourcearea.commands = {
          source: {
            ...
            exec: function(editorInstance) {
              ...
              editorInstance.setMode(
                editorInstance.mode === 'source' ? 'wysiwyg' : 'source'
              );
            },
            canUndo: false
          },
          searchCode: {
            ...
            exec: function(editorInstance) {
              CodeMirror.commands.find(
                window['codemirror_' + editorInstance.id]
              );
            },
            canUndo: true
          },
          ...
          commentSelectedRange: {
            ...
            exec: function(editorInstance) {
              ...
              window['codemirror_' + editorInstance.id].commentRange(
                true,
                range.from,
                range.to
              );
          ...
          uncommentSelectedRange: {
            ...
            exec: function(editorInstance) {
              ...
              if (
                window['codemirror_' + editorInstance.id].config
                  .autoFormatOnUncomment
              ) {
                window['codemirror_' + editorInstance.id].autoFormatRange(
                  range.from,
          ...
          autoCompleteToggle: {
            ...
            exec: function(editorInstance) {
              if (this.state === CKEDITOR.TRISTATE_ON) {
                window['codemirror_' + editorInstance.id].setOption(
                  'autoCloseTags',
                  false
                );
      ...
      function getCodeMirrorScripts() {
        ...
        switch (config.mode) {
          ...
          case 'bbcodemixed':
            {
              scriptFiles.push(
                rootPath + 'js/codemirror.mode.bbcodemixed.min.js'
              );
            }
          ...
          case 'htmlmixed':
            {
              scriptFiles.push(
                rootPath + 'js/codemirror.mode.htmlmixed.min.js'
              );
            }
          ...
          case 'text/javascript':
            {
              scriptFiles.push(
                rootPath + 'js/codemirror.mode.javascript.min.js'
              );
            }
      ...
      function loadCodeMirror(editor) {
        ...
        textarea.setStyles(
          CKEDITOR.tools.extend(
            ...
            CKEDITOR.tools.cssVendorPrefix(
              'tab-size',
              editor.config.sourceAreaTabSize || 4
            )
          )
        );
        ...
        window['editable_' + editor.id] = editor.editable(
          new sourceEditable(editor, textarea)
        );
        ...
        // Enable Code Folding (Requires 'lineNumbers' to be set to 'true')
        if (config.lineNumbers && config.enableCodeFolding) {
          window['foldFunc_' + editor.id] = CodeMirror.newFoldFunction(
            CodeMirror.tagRangeFinder
          );
        }
        ...
        var holderHeight =
          holderElement.$.clientHeight === 0
            ? editor.ui.space('contents').getStyle('height')
            : holderElement.$.clientHeight + 'px';
        ...
        if (config.autoFormatOnStart) {
          if (config.useBeautifyOnStart) {
            ...
            window['codemirror_' + editor.id].setValue(
              html_beautify(source, indent_size, indent_char, 120, brace_style)
            );
          } else {
            window['codemirror_' + editor.id].autoFormatAll(
              {
      ...
      if (config.enableCodeFormatting) {
        ...
        editor.addCommand(
          'commentSelectedRange',
          sourcearea.commands.commentSelectedRange
        );
        editor.addCommand(
          'uncommentSelectedRange',
          sourcearea.commands.uncommentSelectedRange
        );
        editor.addCommand(
          'autoCompleteToggle',
          sourcearea.commands.autoCompleteToggle
        );

        if (editor.ui.addButton) {
          if (
            config.showFormatButton ||
      ...
      editor.on('beforeModeUnload', function(evt) {
        if (
          ...
          range.startOffset = LineChannelToOffSet(
            window['codemirror_' + editor.id],
            window['codemirror_' + editor.id].getCursor(true)
          );
          range.endOffset = LineChannelToOffSet(
            window['codemirror_' + editor.id],
            window['codemirror_' + editor.id].getCursor(false)
          );
      ...
      editor.on('setData', function(data) {
        if (
          window['editable_' + data.editor.id] &&
  ...
  var sourceEditable = CKEDITOR.tools.createClass({
    ...
    proto: {
      setData: function(data) {
        ...
        if (
          window['editable_' + this.editor.id] &&
          this.editor.mode === 'source'
        ) {
...
function IsStyleSheetAlreadyLoaded(href) {
  return (
    CKEDITOR.document.getHead().findOne('link[href="' + href + '"]') != null
  );
}
