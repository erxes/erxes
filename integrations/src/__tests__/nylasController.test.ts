...
import { NylasCalendars, NylasEvent, NylasGmailConversationMessages } from '../nylas/models';
...
describe('Test nylas controller', () => {
  ...
  afterEach(async () => {
    ...
    await NylasEvent.remove({});
    await NylasCalendars.remove({});
  ...
  test('Get calendars', async () => {
    ...
    const integration = await integrationFactory({ nylasToken: 'token', erxesApiId: 'erxesApiId' });

    const [calendar] = await nylasGetCalendars(integration.erxesApiId);
    ...
    try {
      await nylasGetCalendars(integration.erxesApiId);
    } catch (e) {
  ...
  test('Get calendar events', async () => {
    try {
      await nylasGetAllEvents('askld');
    } catch (e) {
      expect(e.message).toBe('Integration not found with id: askld');
    }
    ...
    const mock1 = 
      sinon.stub(api, 'getCalenderOrEventList')
      .returns(Promise.resolve([
        {
          id: '321',
    ...
    await nylasGetAllEvents(integration.erxesApiId);

    const event = await NylasEvent.findOne({ providerCalendarId: calendar.providerCalendarId });
    ...
    const mock2 = sinon.stub(api, 'getCalenderOrEventList')
  ...
  test('Get calender or event', async () => {
    ...
    const mock1 = sinon.stub(api, 'getCalendarOrEvent')
      .returns(Promise.resolve({ status: 'success' }));

    const integration = await integrationFactory({ erxesApiId: 'erxesApId' });

    const response = await nylasGetCalendarOrEvent('id', 'calendars', integration.erxesApiId);
  ...
  test('Check calendar availability', async () => {
    try {
      await nylasCheckCalendarAvailability('erxesApiId123', { startTime: 1, endTime: 2});
    } catch (e) {
    ...
    const mock1 = sinon.stub(api, 'checkCalendarAvailability')
      .returns(Promise.resolve([{object: 'free_busy'}]));

    const [response] = await nylasCheckCalendarAvailability(integration.erxesApiId, { startTime: 1, endTime: 2});

    expect(response.object).toBe('free_busy');

    mock1.restore();

    const mock2 = sinon.stub(api, 'checkCalendarAvailability')
      .throws(new Error('error'));

    try {
      await nylasCheckCalendarAvailability(integration.erxesApiId, { startTime: 1, endTime: 2});
    } catch (e) {
  ...
  test('Delete calendar event', async () => {
    ...
    const mock1 = sinon.stub(api, 'deleteCalendarEvent')
      .returns(Promise.resolve({ status: 'success' }));
    ...
    const mock2 = sinon.stub(api, 'deleteCalendarEvent')
      .throws(new Error('error'));
  ...
  test('Create calendar event', async () => {
    ...
    try {
      await nylasCreateCalenderEvent({ erxesApiId: 'erxesApiId123', ...args });
    } catch (e) {
      expect(e.message).toBe('Integration not found with id: erxesApiId123');
    }
    ...
    const mock1 = sinon.stub(api, 'createEvent')
    .returns(Promise.resolve({ status: 'success' }));

    await nylasCreateCalenderEvent({ erxesApiId: integration.erxesApiId, ...args });

    mock1.restore();

    const mock2 = sinon.stub(api, 'createEvent')
      .throws(new Error('error'));
  ...
  test('Update event', async () => {
    ...
    const mock1 = sinon.stub(api, 'updateEvent')
    .returns(Promise.resolve({ status: 'success' }));

    await nylasUpdateEvent({ erxesApiId: integration.erxesApiId, ...args });

    mock1.restore();

    const mock2 = sinon.stub(api, 'updateEvent')
      .throws(new Error('error'));
  ...
  test('Send event attendance', async () => {
    ...
    const mock1 = sinon.stub(api, 'sendEventAttendance')
    .returns(Promise.resolve({ status: 'success' }));

    await nylasSendEventAttendance({ erxesApiId: integration.erxesApiId, ...args });

    mock1.restore();

    const mock2 = sinon.stub(api, 'sendEventAttendance')
      .throws(new Error('error'));
  ...
  test('Sync event', async () => {
    try {
      ...
      expect(e.message).toBe('Integration not found with accountUid: accountUid')
    }
    ...
    const mockEventCreate = sinon.stub(api, 'getCalendarOrEvent')
      .returns(Promise.resolve({
        id: 'eventId',
    ...
    await nylasUtils.syncEvents('event.created', integration.nylasAccountId, 'eventId');
      
    const eventCreated = await NylasEvent.findOne({ providerEventId: 'eventId' });
    ...
    const mockEventUpdated = sinon.stub(api, 'getCalendarOrEvent')
      .returns(Promise.resolve({
        id: 'eventId',
    ...
    await nylasUtils.syncEvents('event.updated', integration.nylasAccountId, 'eventId');
    ...
    const mockEventUpdatedFail = sinon.stub(api, 'getCalendarOrEvent')
    ...
    await nylasUtils.syncEvents('event.deleted', integration.nylasAccountId, 'eventId123');

    expect(await NylasEvent.findOne({ providerEventId: 'eventId123' })).toBe(null);

    const mockFail = sinon.stub(api, 'getCalendarOrEvent')
      .throws(new Error('error'))
    ...
    try {
      await nylasUtils.syncEvents('event.created', integration2.nylasAccountId, 'eventId123');
    } catch (e) {
      expect(e.message).toBe('error')
    }
  ...
  test('Sync calendars', async () => {
    try {
      await nylasUtils.syncCalendars('calendar.created', 'accountUid', 'calendarId');
    } catch (e) {
      expect(e.message).toBe('Integration not found with accountUid: accountUid')
    }
    ...
    const mockCalendarCreate = sinon.stub(api, 'getCalendarOrEvent')
      .returns(Promise.resolve({
    ...
    await nylasUtils.syncCalendars('calendar.created', integration.nylasAccountId, 'calendarId123123v');
      
    const calendarCreated = await NylasCalendars.findOne({ providerCalendarId: 'calendarId123123v' });

    expect(calendarCreated.providerCalendarId).toBe('calendarId123123v');

    mockCalendarCreate.restore();

    const mockCalendarUpdate = sinon.stub(api, 'getCalendarOrEvent')
      .returns(Promise.resolve({
        id: 'calendarId123123v',
        name: 'updated name'
      }));

    await nylasUtils.syncCalendars('calendar.updated', integration.nylasAccountId, 'calendarId123123v');

    const calendarUpdated = await NylasCalendars.findOne({ providerCalendarId: 'calendarId123123v' });
    ...
    const mockCalendarUpdateFail = sinon.stub(api, 'getCalendarOrEvent')
      .returns(Promise.resolve({
        id: 'caleId123123v',
        name: 'updated name'
      }));
    ...
    try {
      ...
      await nylasUtils.syncCalendars('calendar.updated', integration.nylasAccountId, 'caleId123123v');
    } catch (e) {
    ...
    await nylasUtils.syncCalendars('calendar.deleted', integration.nylasAccountId, 'calendarId123123v');

    expect(await NylasCalendars.findOne({ providerCalendarId: 'calendarId123123v' })).toBe(null);
    expect(await NylasEvent.findOne({ providerCalendarId: 'calendarId123123v' })).toBe(null);
