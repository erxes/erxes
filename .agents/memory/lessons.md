...
## 2026-05-22 — Branch name collision with abandoned-PR branches blocks fresh-wish retries
...
**Root cause:** Abandoned PRs leave their feature branches in place locally. A "Fresh wish" retry of the same feature naturally wants the same branch name — collision is inevitable when the system encourages mirroring sister precedent by name (deal-confidence-score → confidenceScore).
**Lesson:** When starting Phase 5 for a wish whose slug matches a known prior attempt (`memory/lessons.md` mentions a PR by name), prepend the wish-ID date to the branch name: `feat/<wish-id>` (e.g. `feat/2026-05-22-deal-confidence-score`). This preserves the abandoned branch as forensic evidence and makes lineage obvious in `git branch -a`. Do **not** `-D` the existing branch — it is a destructive op that needs explicit developer approval, and the branch is the only record of the prior attempt's exact diff.
...
**Where applicable:** `WORKFLOW.md` Phase 3 GROUND ("check for stale spec references" sub-step); `WORKFLOW.md` Phase 6 (allow proactive cleanup of dangling refs from the same surface). Sister precedent for the same area is the canonical place to leave this evidence — future wishes auto-grep it.

## 2026-05-22 — Fixture-seeder is a multi-wish dependency, not an inline Phase 6 task
**Symptom:** Phase 6 of `2026-05-22-deal-confidence-score` discovered that NO test-fixture infrastructure exists in `.agents/`: no auth helper, no GraphQL client, no seeder for board/pipeline/stage/deal, no teardown. Building it inline would have ballooned the wish from ~190 LOC of production code to ~600 LOC including infra.
**Root cause:** The lesson `2026-05-22 — Phase 6 VERIFY allowed test.skip cop-outs` correctly identified the slop pattern. It did NOT specify *where the seeder lives* or *who builds it*. Each downstream wish then faced the same infra-gap.
**Lesson (AMENDED 2026-05-22 — see "behavior-coverage floor closes the named-skip meta-cop-out" below):** Test-fixture infrastructure is a wish in its own right (`2026-05-22-test-fixture-seeder/WISH.md`). The named-blocking-wish form is acceptable **for individual behavior criteria**, but **NOT for every behavior criterion in a wish** — that's the meta-cop-out where a downstream wish gets "all my tests are skipped against one yet-to-be-built wish" and ships unverified. Phase 6 now requires at least one behavior-bucket test to be non-skipped, seeded inline, passing. If every behavior criterion in a wish would have to point at the same blocking infra wish, the wish has discovered an infra dependency — stop and ship the seeder first, or reshape the wish.
**Where applicable:** `WORKFLOW.md` Phase 6 (the behavior-coverage floor); `SLOP-CHECKLIST.md` (the new "meta-cop-out" entry); `2026-05-22-test-fixture-seeder/WISH.md` (still the legitimate target for *some* skips, just not all).

## 2026-05-22 — Behavior-coverage floor closes the named-skip meta-cop-out
**Symptom:** `2026-05-22-deal-confidence-score` shipped under the original Phase 6 rule with 2 wiring tests (live-gated) + 7 behavior tests (`test.skip(true, 'BLOCKED on wish 2026-05-22-test-fixture-seeder')`). Every behavior criterion in the SPEC was deferred to one infra wish. The original Phase-6 rule "skip is legal if it names a blocking wish" treated each skip in isolation; the wish-level pattern of "all skips → one wish" snuck through.
**Root cause:** A rule that's stated per-test fails to catch a pattern that's per-wish. The named-blocking-wish marker became the path of least resistance, exactly the slop the workflow was supposed to prevent.
**Lesson:** Classify SPEC acceptance criteria upfront into **wiring** (live-gated, no seed) and **behavior** (seed required). Require at least one behavior-bucket criterion to be non-skipped, seeded, passing. If the entire behavior surface would be skipped against the same wish, Phase 6 halts. The `add-deal-field.md` skill's Phase 6 section has been updated with this matrix.
**Where applicable:** `WORKFLOW.md` Phase 6; `SLOP-CHECKLIST.md` "meta-cop-out" entry; `templates/SPEC.md` Test-coverage matrix; `templates/REVIEW.md` behavior-coverage floor check; every skill's Phase 6 section.

## 2026-05-22 — Wish status `shipped` requires a merge SHA; AI never self-assigns it
**Symptom:** `WISH.md` for `2026-05-22-deal-confidence-score` carried `Status: shipped (awaiting PR open)` — a logically impossible state. `gh pr list --head feat/2026-05-22-deal-confidence-score` returned `[]`. Branch was pushed but PR was not opened.
**Root cause:** The status field was a free-text string with no state machine. AI could narrate any value. The orchestrator's Phase 7 said "open the PR" but didn't gate on `gh pr create` actually succeeding and the URL being captured into an artifact.
**Lesson:** A status enum with required artifacts per state. AI never self-assigns `shipped`; only the merge commit on `main` does. AI may set `pr-open` only after writing `SHIP.md` with a `gh pr view`-verifiable URL. The legal enum is in `WORKFLOW.md` "Status state machine." Hard rule #13 in `SYSTEM-PROMPT.md`.
**Where applicable:** `WORKFLOW.md` Status state machine; `SYSTEM-PROMPT.md` hard rule #13; `templates/WISH.md` Status field; `.claude/commands/sales.md` Phase 7 (SHIP.md gate); `.claude/commands/wishes.md` + `wish.md` (status-drift detection).

## 2026-05-22 — Per-commit eval runs need a captured audit trail (EVAL.log)
**Symptom:** `2026-05-22-deal-confidence-score` REVIEW.md narrates "evals/run.sh sales exit 0" with one aggregate output snippet — no per-commit lines. PLAN.md said each commit ran the eval; reality may match, but the artifact is a single narration in REVIEW, not 9 verifiable lines.
**Root cause:** Phase 5 of WORKFLOW.md said "run the eval after each commit" but didn't require capturing the result into a durable artifact. "I ran it 9 times" was a claim, not a proof.
**Lesson:** Phase 5 now writes `EVAL.log` — one line per `evals/run.sh sales` invocation, including failures and retries, plus a final aggregate. REVIEW.md confirms its existence. Without EVAL.log, Phase 7 cannot proceed (the per-commit gate is unverifiable).
**Where applicable:** `WORKFLOW.md` Phase 5; `.claude/commands/sales.md` Phase 5 + Phase 6; `templates/PLAN.md`, `templates/REVIEW.md`.

## 2026-05-22 — Two files with the same basename in sibling dirs is cognitive collision (slop)
**Symptom:** `2026-05-22-deal-confidence-score` created `components/deal-selects/SelectConfidenceScore.tsx` and `components/common/filters/SelectConfidenceScore.tsx`. Both exist; both have one clear purpose; both come up when a reader greps `SelectConfidenceScore`. The reader has to read the path to disambiguate.
**Root cause:** Mirrored the `priority` precedent 1:1, which itself violates this. The SPEC originally named the filter file `FilterConfidenceScore.tsx`; implementation diverged silently to inherit the (slop) precedent.
**Lesson:** Filenames are unique within a plugin module. Distinct surfaces get distinct basenames (`SelectX.tsx` for edit, `FilterX.tsx` for filter) OR one file namespaces both via `Object.assign(Root, { FilterBar, FilterView, FilterItem })`. The `priority` precedent is grandfathered slop; new wishes don't inherit it. New SLOP-CHECKLIST entry + `rules/10-code-style.md` "File naming" section.
**Where applicable:** `SLOP-CHECKLIST.md` "Two files with the same basename"; `rules/10-code-style.md` File naming; `skills/sales/add-deal-field.md` "Filename uniqueness" subsection.
