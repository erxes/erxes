...
**Root cause:** The lesson `2026-05-22 — Phase 6 VERIFY allowed test.skip cop-outs` correctly identified the slop pattern and forbade the cop-out. It did NOT specify *where the seeder lives* or *who builds it*. Each downstream wish then faced the same infra-gap, with no path to fulfillment.
**Lesson:** Test-fixture infrastructure is a wish in its own right (`2026-05-22-test-fixture-seeder/WISH.md` — captured but not yet planned). It is the only legitimate `BLOCKED on wish <id>` named-skip target. Until that wish ships, the named-skip form is the workflow's correct escape hatch — NOT slop. Phase 6 mandate should be read as: "skip with a named blocking wish, OR seed yourself, OR write live-stack-gated tests where the assertion doesn't need seeded data." All three are valid forms.
**Where applicable:** `WORKFLOW.md` Phase 6 (clarify the three valid test forms); `2026-05-22-test-fixture-seeder/WISH.md` (the blocker — un-skipping the SPEC tests on `deals.spec.ts` is its deliverable). When a future wish references this blocking wish, treat the reference as a real dependency, not an escape hatch.
