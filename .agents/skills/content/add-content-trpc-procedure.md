# add content trpc procedure

> **When to use:** the wish exposes content data/behavior to **another plugin** via type-safe RPC — e.g., `content.cms.cmssForCustomer(customerId)` so frontline can show "open cmss" inside a conversation card, or `content.pos.activeTerminals()` so operation can list POS-affected branches. NOT for UI-only data flow (that's GraphQL) and NOT for fire-and-forget events (that's Redis pubsub — see [`../../rules/20-architecture-boundaries.md`](../../rules/20-architecture-boundaries.md)).

## Phase 3 — GROUND (mirror an existing feature)

**Step 1 (mandatory): find the sister feature you will mirror.**

Closest sisters in `backend/plugins/content_api/src/modules/content/trpc/cms.ts`:
...
- `backend/plugins/content_api/src/modules/content/trpc/cms.ts` — the full `cmsTrpcRouter`. Pay attention to the `t.procedure.input(z.any()).query/mutation(...)` shape and the `{ status: 'success', data: ... }` response convention
- `backend/plugins/content_api/src/modules/pos/trpc/pos.ts` — sibling pos router showing the same pattern
- `backend/plugins/content_api/src/trpc/init-trpc.ts` — `appRouter` merges all sub-routers. New top-level namespaces register here.
- [`../../rules/20-architecture-boundaries.md`](../../rules/20-architecture-boundaries.md) — when to use tRPC vs GraphQL federation vs pubsub
- `backend/erxes-api-shared/src/utils/*` — look for `sendTRPCMessage` (used in `cms.ts` line 430+) to understand the caller side

If your procedure needs to call **another plugin's** tRPC (e.g., content calls core to fetch a customer), use the `sendTRPCMessage` pattern shown in `cms.ts` `fetchSegment` (lines 430–450).

## Phase 4 — PLAN

Default plan for adding a new procedure under an existing namespace (e.g., `cms.cmssForCustomer`):

1. **add procedure to existing router** — files: `backend/plugins/content_api/src/modules/content/trpc/cms.ts`
2. **(optional) extract shared logic** — only if a second caller emerges, otherwise inline ([`../../SLOP-CHECKLIST.md`](../../SLOP-CHECKLIST.md))
3. **playwright/eval** — files: `.agents/plugins/content/tests/cmsList.spec.ts`

If the procedure is a brand-new namespace under content (e.g., a `forecast.*` set of procedures), add one more commit:

4. **register namespace in init-trpc.ts** — files: `backend/plugins/content_api/src/trpc/init-trpc.ts`
...
- **Option A (preferred):** add a unit/integration test in `backend/plugins/content_api/src/modules/content/__tests__/<procedure>.test.ts` that builds a tRPC caller, runs the procedure against a seeded DB, and asserts the response. Mirror existing tests under `__tests__/` if any.
- **Option B (when the consuming plugin is in this repo):** find the caller (`sendTRPCMessage({ pluginName: 'content', ... })`), write/update a Playwright spec for the consumer's UI surface that exercises the cross-plugin path end to end.
...
- **`try/catch` discipline.** The sister `createItem` and `editItem` (cms.ts lines 162–172, 187–204) wrap their work in `try/catch` because they're a public boundary that returns `{ status: 'error', errorMessage }`. That's the only legitimate `try/catch` shape in content tRPC. Don't add `try/catch` to a read that can't fail.
...
- [ ] No new namespace if the procedure fits an existing one (`cms`, `stage`, `pipeline`, `pos`)
