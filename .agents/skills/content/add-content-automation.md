...
|---|---|---|
| `content:cms` (default trigger) | event-on-create | `meta/automations/constants.ts` line 4–10 |
| `content:cms` with `relationType: 'probability'` | custom event with extra check | constants line 11–20 + `trigger/checkStageProbalityTrigger.ts` |
| `create` on `content:cms` | action that creates a content entity | constants line 23–33 + `action/createAction.ts` |
| `create` on `content:checklist` | action variant | constants line 34–42 + `action/createChecklist.ts` |

**Read these files in full** before writing any code:

- `backend/plugins/content_api/src/meta/automations.ts` — top-level aggregate (`constants`, `receiveActions`, `checkCustomTrigger`, `setProperties`, `replacePlaceHolders`). Producer-handler pattern via `createCoreModuleProducerHandler`.
- `backend/plugins/content_api/src/modules/content/meta/automations/constants.ts` — both `triggers[]` and `actions[]`, plus the `isCustom`, `isTargetSource`, `targetSourceType` flags
- `backend/plugins/content_api/src/modules/content/meta/automations/automationHandlers.ts` — how `checkCustomTrigger` and `receiveActions` dispatch by `collectionType`/`relationType`
- For a custom trigger: `backend/plugins/content_api/src/modules/content/meta/automations/trigger/checkStageProbalityTrigger.ts` — the full "trigger predicate" pattern
- For an action: `backend/plugins/content_api/src/modules/content/meta/automations/action/createAction.ts` and `createChecklist.ts` — the receive-actions side
- `backend/plugins/content_api/src/modules/content/meta/automations/action/getRelatedValue.ts` and `getItems.ts` — placeholder resolution + related-item fetch
...
1. **register trigger in `constants.ts`** — files: `backend/plugins/content_api/src/modules/content/meta/automations/constants.ts`
2. **(custom trigger only) add predicate** — files: `backend/plugins/content_api/src/modules/content/meta/automations/trigger/<your>Trigger.ts`
3. **wire predicate into `automationHandlers.checkCustomTrigger`** — files: `automationHandlers.ts`
4. **(if it needs custom UI config) extend the automations widget remote** — files: `frontend/plugins/content_ui/src/widgets/automations/modules/content/...`
5. **playwright/automation eval** — files: `.agents/plugins/content/tests/cmsList.spec.ts` (assert the trigger label appears in the automation picker, if reachable from the UI)

For a new action:

1. **register action in `constants.ts`**
2. **add handler file** — `backend/plugins/content_api/src/modules/content/meta/automations/action/<your>Action.ts`
...
Add to `.agents/plugins/content/tests/cmsList.spec.ts` (or a new `automations.spec.ts` if the suite is missing):

- a test that the new trigger/action label appears wherever the automation picker lists content options (host-side, but reachable from the content UI)
- a behavioral test: with seeded data, fire the trigger condition (or run the action) and assert the side effect — a created cms, a created checklist item, etc.

Run: `cd .agents && pnpm test plugins/content/tests/cmss.spec.ts`

If automations aren't reachable from the content UI under test (they often live in the core automations builder), use `test.skip` with a documented reason rather than faking the assertion.
