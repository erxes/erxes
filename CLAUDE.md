...
| Wishes a Frontline feature ("add X to Conversation", "show Y on the inbox", "filter by Z") | Run `/frontline "<wish>"` — orchestrator at [`.claude/commands/frontline.md`](./.claude/commands/frontline.md). Drives the 7-phase workflow at [`.agents/WORKFLOW.md`](./.agents/WORKFLOW.md). |
| Wishes an Operation feature ("add X to Task", "show Y on the timeline", "filter by Z") | Run `/operation "<wish>"` — orchestrator at [`.claude/commands/operation.md`](./.claude/commands/operation.md). Drives the 7-phase workflow at [`.agents/WORKFLOW.md`](./.agents/WORKFLOW.md). |
| Wishes a feature in another plugin (accounting, etc.) | The system is **sales, frontline, and operation-only today**. Tell the developer; do not invent a workflow. See [`.agents/EXTENDING.md`](./.agents/EXTENDING.md) for how the sales pattern will be replicated. |
