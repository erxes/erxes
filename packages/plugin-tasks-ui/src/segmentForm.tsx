...
import { BoardsQueryResponse } from '@erxes/ui-tasks/src/boards/types';
...
import { queries } from '@erxes/ui-tasks/src/settings/boards/graphql';
...
class Form extends React.Component<any, any, any> {
  ...
  render() {
    ...
    if (component === 'filter') {
      if (propertyType && !['tasks:task'].includes(propertyType)) {
        return null;
      }

      if (!hideDetailForm && ['tasks:task'].includes(type)) {
        return null;
      }

      return content;
    } else if (['tasks:task'].includes(type)) {
      return content;
    }
  }
}
