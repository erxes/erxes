...
const saveJournals = async (models, messageBroker, contract, config, journalConfig, customerCode) => {
  const startDate = getPureDate(contract.startDate).toISOString().slice(0, 10)
  const postConfig = {
  ...
  if (contract.tenor > 12) {
    const insuranceTypeIds = [...new Set(contract.collateralsData.map(item => item.insuranceTypeId))];
    const insuranceTypes = await models.InsuranceTypes.find({ _id: { $in: insuranceTypeIds } }).lean();
    const insuranceCompanyIds = [...new Set(insuranceTypes.map(item => item.companyId))];
    const insuranceCompanies = await models.Companies.find({ _id: { $in: insuranceCompanyIds } });
    ...
    for (const coll of contract.collateralsData) {
      const insuAmount = (contract.tenor - 12) / 12 * coll.insuranceAmount || 0;
      if (!insuAmount) {
        continue
      }
  ...
  const schedules = await models.RepaymentSchedules.find({ contractId: contract._id });
  ...
  const futureInterest = futureInterests.interestEve + futureInterests.interestNonce;
...
const mainPayEbarimt = async (models, messageBroker, contract, config, journalConfig, customerCode, billType) => {
  ...
  const orderInfos = [{
    date: getPureDate(contract.startDate).toISOString().slice(0, 10),
    orderId: contract._id,
...
export const ConfirmBase = async (models, messageBroker, memoryStorage, contract: IContractDocument) => {
  ...
  // on Cost
  const responseJournals = await saveJournals(models, messageBroker, contract, config, journalConfig, customerCode);
  ...
  const responseMainPay = await mainPayEbarimt(models, messageBroker, contract, config, journalConfig, customerCode, billType);
