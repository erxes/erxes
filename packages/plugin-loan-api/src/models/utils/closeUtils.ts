...
export const getCloseInfo = async (models, memoryStorage, contract: IContractDocument, date: Date) => {
  const closeDate = getFullDate(date);
  const contractId = contract._id;

  let lastPaySchedule = await models.RepaymentSchedules.findOne({
    contractId, status: { $in: [SCHEDULE_STATUS.DONE, SCHEDULE_STATUS.LESS] }
  }).sort({ payDate: -1 }).lean();
  ...
  const { undue, interestEve, interestNonce, insurance } = await getCalcedAmounts(
    models, memoryStorage, { contractId, payDate: closeDate }
  ) as any;

  const pendingSchedules = await models.RepaymentSchedules.find({
    contractId, payDate: { $gt: lastPaySchedule.payDate }, debt: { $exists: true }
  })

  const debt = pendingSchedules.length ? pendingSchedules.reduce((a, c) => {
  ...
  const result = {
    ...
    debt,
    total: lastPaySchedule.balance + (undue || 0) + (interestEve || 0) + (interestNonce || 0) + (insurance || 0) + debt,
  }
