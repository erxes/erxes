...
import { ICalcDivideParams, ICalcTrParams, ITransactionDoc } from '../definitions/transactions';
import {
  afterNextScheduled, betweenScheduled, onNextScheduled, onPreScheduled
} from './scheduleUtils';
import { calcInterest, getUnduePercent, getDiffDay, getFullDate, getDatesDiffMonth } from './utils';
...
export const getAOESchedules = async (models, contract) => {
  ...
  const preSchedule = await models.RepaymentSchedules.findOne({
    contractId: contract._id, status: { $in: [SCHEDULE_STATUS.DONE, SCHEDULE_STATUS.LESS] }
  }).sort({ payDate: -1 }).lean();

  const nextSchedule = await models.RepaymentSchedules.findOne({
    contractId: contract._id, status: { $in: [SCHEDULE_STATUS.PENDING, SCHEDULE_STATUS.LESS] }
  }).sort({ payDate: 1 }).lean();

  return { preSchedule, nextSchedule }
}

export const getCalcedAmounts = async (models, memoryStorage, doc: ICalcDivideParams) => {
  const result = {
    undue: 0,
    interestEve: 0,
    interestNonce: 0,
  ...
  // closed contract
  if (!nextSchedule) {
    const unduePercent = await getUnduePercent(models, memoryStorage, preSchedule.payDate) || contract.unduePercent || 0.2
    result.undue = Math.round(preSchedule.balance / 100 * unduePercent * getDiffDay(prePayDate, trDate));
  ...
  if (getDiffDay(trDate, prePayDate) === 0) {
    ...
    result.interestEve = (preSchedule.interestEve || 0) - (preSchedule.didInterestEve || 0);
    result.interestNonce = (preSchedule.interestNonce || 0) - (preSchedule.didInterestNonce || 0);
  ...
  if (trDate < nextPayDate) {
    ...
    const percentOfDay = getDiffDay(preSchedule.payDate, trDate) / getDiffDay(preSchedule.payDate, nextSchedule.payDate);
  ...
  // after
  const unduePercent = await getUnduePercent(models, memoryStorage, preSchedule.payDate) || contract.unduePercent || 0.2
  result.undue = Math.round(preSchedule.balance / 100 * unduePercent * getDiffDay(nextPayDate, trDate));
  const { diffEve, diffNonce } = getDatesDiffMonth(prePayDate, trDate);
  ...
  const doubleSkipped = await models.RepaymentSchedules.find({ _id: { $ne: nextSchedule }, $and: [{ payDate: { $gte: nextSchedule.payDate } }, { payDate: { $lte: trDate } }] })
...
export const transactionRule = async (models, memoryStorage, doc: ICalcTrParams) => {
  const result = {
    payment: 0, undue: 0, interestEve: 0, interestNonce: 0, insurance: 0, debt: 0, surplus: 0, calcedInfo: undefined
  }
  ...
  result.calcedInfo = await getCalcedAmounts(
    models, memoryStorage, { contractId: doc.contractId, payDate: doc.payDate }
  );

  const { payment = 0, undue = 0, interestEve = 0, interestNonce = 0, insurance = 0, debt = 0, preSchedule } = result.calcedInfo;
  result.calcedInfo.total = payment + undue + interestEve + interestNonce + insurance + debt;
...
export const trAfterSchedule = async (models, tr: ITransactionDoc) => {
  if (!tr.contractId) {
    return;
  }
  ...
  const contract = await models.LoanContracts.getContract(models, { _id: tr.contractId });
  ...
  const pendingSchedules = await models.RepaymentSchedules.find({
    contractId: contract._id, status: SCHEDULE_STATUS.PENDING
  }).sort({ payDate: 1 }).lean();
  ...
  // on schedule
  if (getDiffDay(trDate, nextPayDate) === 0) {
    await onNextScheduled(models, contract, tr, preSchedule, nextSchedule, pendingSchedules);
    return;
  }

  // delayed
  await afterNextScheduled(models, contract, tr, preSchedule, nextSchedule, pendingSchedules);
  return;
}

export const removeTrAfterSchedule = async (models, tr, noDeleteSchIds = []) => {
  if (!Object.keys(tr.reactions || {}).length) {
    return;
  }

  const nextTrs = await models.LoanTransactions.find({ contractId: tr.contractId, payDate: { $gt: tr.payDate } }).lean();
  if (nextTrs && nextTrs.length) {
    throw new Error('this transaction is not last transaction. Please This contracts last transaction only to change or remove')
  }

  if (tr.invoiceId) {
    await models.LoanInvoices.updateInvoice(models, tr.invoiceId, { status: INVOICE_STATUS.PENDING })
  }
