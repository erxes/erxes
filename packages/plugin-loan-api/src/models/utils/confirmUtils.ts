...
export const checkCompanyRd = async (models, contract, config) => {
  const conformities = await models.Conformities.getConformities({ mainType: 'contract', mainTypeIds: [contract._id], relTypes: ['customer', 'company'] });
  ...
  const companyIds = conformities.map(item => {
    if (item.mainType === 'company')
      return item.mainTypeId
    if (item.relType === 'company')
  ...
  if (billType === 1) {
    const customerIds = conformities.map(item => {
      if (item.mainType === 'customer')
        return item.mainTypeId
      if (item.relType === 'customer')
        return item.relTypeId
    })
...
export const sentErkhet = async (messageBroker, postData, isJournal?, isEbarimt?) => {
  return messageBroker().sendRPCMessage(
    'rpc_queue:erxes-automation-erkhet',
    {
      action: isJournal ? 'get-response-send-journal-orders' : 'get-response-send-order-info',
