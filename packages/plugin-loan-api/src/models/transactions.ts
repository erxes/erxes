...
import { removeTrAfterSchedule, trAfterSchedule, transactionRule } from './utils/transactionUtils'
...
export class Transaction {
  ...
  public static async getTransaction(models, selector: any) {
   ...
   */
  public static async createTransaction(models, messageBroker, memoryStorage, doc) {
    doc = { ...doc, ...(await findContractOfTr(models, doc)) };

    const contract = await models.LoanContracts.findOne({ _id: doc.contractId }).lean();
    if (!contract || !contract._id) {
      return models.LoanTransactions.create({ ...doc });
    }

    if (doc.invoiceId) {
      await models.LoanInvoices.updateInvoice(models, doc.invoiceId, { status: INVOICE_STATUS.DONE })
    }
   ...
   * Update Transaction
   */
  public static async updateTransaction(models, messageBroker, memoryStorage, _id, doc) {
    doc = { ...doc, ...(await findContractOfTr(models, doc)) };

    const oldTr = await models.LoanTransactions.getTransaction(models, { _id });
    ...
    await models.LoanTransactions.updateOne({ _id }, { $set: { ...doc, ...trInfo } });
    const newTr = await models.LoanTransactions.getTransaction(models, { _id });
   ...
   */
  public static async changeTransaction(models, messageBroker, memoryStorage, _id, doc) {
    const oldTr = await models.LoanTransactions.getTransaction(models, { _id });

    const contract = await models.LoanContracts.findOne({ _id: oldTr.contractId }).lean();
    if (!contract || !contract._id) {
      await models.LoanTransactions.updateOne({ _id }, { $set: { ...doc } });
      return models.LoanTransactions.getTransaction(models, { _id });
    }
    ...
    const noDeleteSchIds = preSchedules.map(item => (item._id)).concat([oldSchedule._id]);

    let trReaction = oldTr.reactions.filter(item => (noDeleteSchIds.includes(item.scheduleId)));

    await removeTrAfterSchedule(models, oldTr, noDeleteSchIds);

    const newTotal = (doc.payment || 0) + (doc.interestEve || 0) + (doc.interestNonce || 0) + (doc.undue || 0) + (doc.insurance || 0) + (doc.debt || 0);

    await models.LoanTransactions.updateOne({ _id }, { $set: { ...doc, total: newTotal } });
    let newTr = await models.LoanTransactions.getTransaction(models, { _id });

    const newBalance = oldSchedule.balance + oldSchedule.didPayment - doc.payment;

    await models.RepaymentSchedules.updateOne({ _id: oldSchedule._id }, {
      $set: {
        payment: doc.payment,
    ...
    let updatedSchedule = await models.RepaymentSchedules.findOne({ _id: oldSchedule._id }).lean();
    ...
    if (oldSchedule.balance !== newBalance) {
      await generatePendingSchedules(models, contract, { ...updatedSchedule }, pendingSchedules, newTr, trReaction);
    }
    ...
    if (doc.futureDebt && doc.debtTenor) {
      ...
      newTr = await models.LoanTransactions.findOne({ _id: newTr._id }).lean();
      ...
      for (const sch of pendingSchedules) {
        ...
        trReaction.push({ scheduleId: sch._id, preData: { debt: sch.debt, total: sch.total } });

        const monthDebt = indexOfMonth === 1 ? firstDebt : perDebt;
        bulkOps.push({
          updateOne: {
            filter: { _id: sch._id },
            update: { $set: { debt: (sch.debt || 0) + monthDebt, total: (sch.total || 0) + monthDebt } }
          }
        })
      }

      await models.RepaymentSchedules.bulkWrite(bulkOps);
      await models.LoanTransactions.updateOne({ _id: newTr._id }, {
   ...
   * Remove Transaction
   */
  public static async removeTransactions(models, _ids) {
