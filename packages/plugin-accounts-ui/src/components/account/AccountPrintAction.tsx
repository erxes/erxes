...
class BulkDocuments extends React.Component<Props, State> {
  constructor(props) {
    ...
    this.state = {
      ...
      copies: 1,
      width: 300,
      branchId: localStorage.getItem('erxes_accounts_documents_branchId') || '',
      departmentId:
        localStorage.getItem('erxes_accounts_documents_departmentId') || ''
    };
  }
  ...
  loadDocuments = () => {
    ...
    client
      .mutate({
        ...
        variables: { contentType: 'accounts' }
      })
      .then(({ data }) => {
  ...
  print = () => {
    ...
    window.open(
      ...
      }/pl:documents/print?_id=${selectedDocumentId}&accountIds=${JSON.stringify(
        bulk.map(b => b._id)
  ...
  onChangeSelect = (name, value) => {
    this.setState({ [name]: value } as any, () => {
      localStorage.setItem(`erxes_accounts_documents_${name}`, value);
    });
  };
