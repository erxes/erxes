...
interface IProps {
  history: any;
  queryParams: any;
  remove: (accountCategoryId: string) => void;
  accountCategories: IAccountCategory[];
  accountCategoriesCount: number;
  loading: boolean;
}

class List extends React.Component<IProps> {
  renderFormTrigger(trigger: React.ReactNode, category?: IAccountCategory) {
    const content = props => (
      <CategoryForm
        {...props}
        category={category}
        categories={this.props.accountCategories}
      />
    );
  ...
  renderEditAction(category: IAccountCategory) {
    const trigger = (
  ...
  renderRemoveAction(category: IAccountCategory) {
  ...
  renderContent() {
    ...
    for (const category of accountCategories) {
      ...
      const name = category.isRoot ? (
        `${category.name} (${category.accountCount})`
      ) : (
        <span>
          {category.name} ({category.accountCount})
        </span>
  ...
  renderCategoryList() {
    const { accountCategoriesCount, loading } = this.props;
    ...
    return (
      <SidebarList>
        <DataWithLoader
          ...
          loading={loading}
          count={accountCategoriesCount}
          emptyText="There is no account category"
          emptyIcon="folder-2"
          size="small"
        />
      </SidebarList>
  ...
  render() {
    return (
      <Sidebar wide={true} hasBorder>
        <Section
          maxHeight={488}
          collapsible={this.props.accountCategoriesCount > 9}
          noMargin
          noShadow
        >
