...
type FinalProps = {
  listActivityTypeQuery: TypeQueryResponse;
} & Props &
  RemoveTypeMutationResponse &
  EditTypeMutationResponse;

const TypesListContainer = (props: FinalProps) => {
  const { listActivityTypeQuery, typesEdit, typesRemove, history } = props;

  if (listActivityTypeQuery.loading) {
    return <Spinner />;
  }
  ...
  const renderButton = ({
    ...
    return (
      <ButtonMutate
        ...
        refetchQueries={['listActivityTypeQuery']}
      />
    );
  };
  ...
  const updatedProps = {
    ...props,
    types: listActivityTypeQuery.activityTypes || [],
    loading: listActivityTypeQuery.loading,
    remove,
    renderButton
  };
...
export default withProps<Props>(
  compose(
    graphql(gql(queries.listActivityTypes), {
      name: 'listActivityTypeQuery',
      options: () => ({
