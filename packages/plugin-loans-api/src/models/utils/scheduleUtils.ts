...
import { CONTRACT_STATUS, LEASE_TYPES, SCHEDULE_STATUS } from '../definitions/constants';
...
const scheduleHelper = async (
  ...
  let skipInterestCalcDate: Date = currentDate;
  if (skipInterestCalcMonth) {
    skipInterestCalcDate = new Date(moment(new Date(currentDate)).add(skipInterestCalcMonth, 'M').format('YYYY-MM-DD'))
  }
  if (skipInterestCalcDay) {
    skipInterestCalcDate = new Date(moment(new Date(currentDate)).add(skipInterestCalcDay, 'day').format('YYYY-MM-DD'))
  }
  ...
  if (contract.stepRules?.length) {
    for (const stepRule of contract.stepRules.sort((a, b) => a.firstPayDate.getTime() - b.firstPayDate.getTime())) {
      ...
      bulkEntries = await scheduleHelper(
        ...
        stepRule.repayment || contract.repayment,
        startDate,
        balance,
        stepRule.interestRate ?? contract.interestRate,
        stepRule.tenor,
        stepRule.salvageAmount || 0,
        contract.weekends || [],
