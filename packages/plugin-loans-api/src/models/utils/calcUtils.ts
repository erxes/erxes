...
import { getAmountByPriority, getCalcedAmountsOnDate, getFreezeAmount } from './calcHelpers';
...
const fixFutureSchedulesLast = async (models, contract, currentSchedule, futureSchedules, currentDate, diffPayment, calculationFixed) => {
  ...
  if (amountInfos.payment < didPayment) {
    ...
    if (futureSchedules.length) {
      ...
      if (!currentSchedule.isDefault) {
        ...
        const nextDiffDay = getDiffDay(tr.payDate, nextSchedule.payDate);
