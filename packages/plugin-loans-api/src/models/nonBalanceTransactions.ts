import {
  ...
  nonBalanceTransactionSchema,
  INonBalanceTransactionDocument,
} from './definitions/nonBalanceTransactions';
import { Model } from 'mongoose';
import { IModels } from '../connectionResolver';
import { FilterQuery } from 'mongoose';

export interface INonBalanceTransactionModel
  extends Model<INonBalanceTransactionDocument> {
  getNonBalanceTransaction(
...
export const loadNonBalanceTransactionClass = (models: IModels) => {
  class NonBalanceTransaction {
    ...
    public static async getNonBalanceTransaction(
      ...
      const NonBalanceTransaction =
        await models.NonBalanceTransactions.findOne(selector);
     ...
     */
    public static async createNonBalanceTransaction(
      subdomain,
      { ...doc }: INonBalanceTransaction,
    ) {
    ...
    public static async removeNonBalanceTransactions(_ids) {
      const nonBalanceTransactions: INonBalanceTransactionDocument[] =
        await models.NonBalanceTransactions.find({ _id: _ids }).lean();

      for await (const oldNonBalTr of nonBalanceTransactions) {
        if (oldNonBalTr) {
          await models.NonBalanceTransactions.deleteOne({
            _id: oldNonBalTr._id,
          });
        }
      }
