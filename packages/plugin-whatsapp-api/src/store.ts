...
export const getOrCreatePostConversation = async (
  ...
  if (!postConversation) {
    ...
    postConversation = await models.PostConversations.create(whatsappPost);
  }
...
export const getOrCreateCustomer = async (
  ...
  const integration = await models.Integrations.findOne({
    $and: [{ whatsappPageId: { $in: pageId } }, { kind }]
  });
  if (!integration) {
  ...
  let firstName;
  try {
    whatsappUser = await getInstagramUser(
      userId,
      facebookPageId || '',
      facebookPageTokensMap
    );
    if (whatsappUser) {
      firstName = whatsappUser.username || whatsappUser.name;
    }
  } catch (e) {
  ...
  if (firstName) {
    try {
      customer = await models.Customers.create({
        ...
        firstName,
        integrationId: integration.erxesApiId,
        profilePic: whatsappUser.profile_pic
      });
    } catch (e) {
      throw new Error(
      ...
      const apiCustomerResponse = await sendInboxMessage({
        ...
        data: {
          ...
          payload: JSON.stringify({
            integrationId: integration.erxesApiId,
            firstName: firstName,
            avatar: whatsappUser.profile_pic,
            isUser: true
          })
        },
        isRPC: true
