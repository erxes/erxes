...
const sendGoal = async (models: IModels, subdomain: string, doc: ISendGoal) => {
  const {
    createdUser,
    receivers,
    title,
  ...
  for (const recipient of recipients) {
    if (recipient.getGoalByEmail && recipient.email) {
  ...
  for (const receiverId of receiverIds) {
    try {
      // send web and mobile goal
      const goal = await models.Goals.createGoal(
        {
          link,
          title,
          content,
      ...
      graphqlPubsub.publish('goalInserted', {
        goalInserted: {
          _id: goal._id,
          userId: receiverId,
          title: goal.title,
...
export const initBroker = async cl => {
  ...
  consumeQueue('goals:send', async ({ subdomain, data }) => {
    const models = await generateModels(subdomain);
    await sendGoal(models, subdomain, data);
  });
  ...
  consumeQueue(
    ...
    async ({ subdomain, data: { selector, modifier } }) => {
      const models = await generateModels(subdomain);
      await models.Goals.update(selector, modifier, { multi: true });
    }
  );
  ...
  consumeRPCQueue(
    ...
    async ({ subdomain, data: { userId, itemId } }) => {
      ...
      return {
        status: 'success',
        data: await models.Goals.checkIfRead(userId, itemId)
      };
    }
  );
  ...
  consumeRPCQueue(
    ...
    async ({ subdomain, data: { selector, fields } }) => {
      ...
      return {
        status: 'success',
        data: await models.Goals.find(selector, fields)
      };
    }
  );
};
