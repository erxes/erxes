...
class EditFormContainer extends React.Component<FinalProps> {
  ...
  componentDidMount() {
    ...
    this.unsubcribe = detailQuery.subscribeToMore({
      document: gql(subscriptions.dealsPipelinesChanged),
      variables: { _id: itemId },
      updateQuery: (
        prev,
        {
          subscriptionData: {
            data: { dealsPipelinesChanged },
          },
        }
      ) => {
        if (!dealsPipelinesChanged || !dealsPipelinesChanged.data) {
          return;
        }

        const { proccessId } = dealsPipelinesChanged;
