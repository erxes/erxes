...
import { WhatsappCommentsQueryResponse, MessagesQueryResponse } from '../types';
...
class ConversationContainer extends React.Component<FinalProps> {
  render() {
    ...
    const messages = messagesQuery.whatsappConversationMessages || [];
    const comments = (commentsQuery && commentsQuery.whatsappGetComments) || [];
...
export default withProps<Props>(
  compose(
    ...
    graphql<Props, MessagesQueryResponse>(
      gql(queries.whatsappConversationMessages),
      {
        name: 'messagesQuery',
        options: ({ conversationId }) => ({
          variables: {
