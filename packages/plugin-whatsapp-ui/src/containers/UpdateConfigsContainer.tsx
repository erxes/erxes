...
const ConfigsContainer = (props: FinalProps) => {
  ...
  const configs = getConfigsQuery.whatsappGetConfigs;
  const configsMap = {};
...
export default withProps<Props>(
  compose(
    graphql(gql(queries.whatsappGetConfigs), {
      name: 'getConfigsQuery',
      options: () => ({
        fetchPolicy: 'network-only',
        variables: { kind: 'whatsapp' }
      })
    }),

    graphql(gql(mutations.whatsappUpdateConfigs), {
