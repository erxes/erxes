import {
  Box, FormControl, Button, FormGroup, ControlLabel, Icon
...
class SocialPaySection extends React.Component<Props, State> {
  ...
  createSocialPay = () => {
    ...
    const mutationName = this.state.withPhone ? mutations.createSPInvoicePhone : mutations.createSPInvoiceQr;
    ...
    client.mutate({
      ...
      const data = withPhone ? response.data.createSPInvoicePhone : response.data.createSPInvoiceQr;
  ...
  onClickEvent = (variable, e) => {
    if (variable === 'invoiceNo') {
      this.setState({ ...this.state, [`${variable}`]: defaultInvoiceNo(32), qr: "", spPaymentStatus: "" });
    }
    else {
      ...
      >
        ...
        <QpaySectionStyle>
          ...
          {spPaymentStatus !== "" && (<>
            ...
            <FormGroup>
              <ControlLabel >Status: {this.state.spPaymentStatus}</ControlLabel>
            </FormGroup>
            <QpayButtonStyle>
              <Button
                btnStyle="simple"
