...
class PaymentSection extends React.Component<Props, State> {
  ...
  render() {
    ...
    const paymentTypes = [{ label: 'Qpay', value: 'qpay' }, { label: 'Social Pay', value: 'socialPay' }];

    return (
      <Box
        title={__('Choose payment type')}
        name="showPayment"
        isOpen={true}
      >
        ...
        <QpaySectionStyle>
          ...
          {this.state.type === "socialPay" && (
            <SocialPaySection amount="10" invoiceNo="socialPay test invoice" phone="91101133" />
          )}
