...
type BoardItemDocument = IGrowthHackDocument;
...
export const putDeleteLog = async (
  ...
  await commonPutDeleteLog(
    subdomain,
    { ...logDoc, description, extraDesc, type: `growthhacks:${logDoc.type}` },
    user
  );
};
...
export const putActivityLog = async (
  ...
  const updatedParams = {
    ...params,
    data: { ...data, contentType: `growthhacks:${data.contentType}` },
  };
  ...
  if (action === 'createBoardItemMovementLog') {
    await sendToWebhook({
      ...
      data: {
        action,
        type: `growthhacks:${data.contentType}`,
        params,
      },
    });
  }
