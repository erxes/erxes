...
type FinalProps = {
  listQuery: CmsQueryResponse;
  listCmsTypeQuery: TypeQueryResponse;
} & Props &
  RemoveMutationResponse &
  EditMutationResponse;
...
const ListContainer = (props: FinalProps) => {
  ...
  const types = listCmsTypeQuery.cmsTypes || [];
  ...
  const remove = cms => {
    confirm('You are about to delete the item. Are you sure? ')
      .then(() => {
        removeMutation({ variables: { _id: cms._id } })
          .then(() => {
  ...
  const updatedProps = {
    ...props,
    cmss: listQuery.cmss || [],
    types: listCmsTypeQuery.cmsTypes || [],
    typeId,
...
export default withProps<Props>(
  compose(
    graphql(gql(queries.listCmsTypes), {
      name: 'listCmsTypeQuery',
      options: () => ({
        fetchPolicy: 'network-only'
      })
    }),

    graphql<Props, CmsQueryResponse, { typeId: string }>(gql(queries.list), {
      name: 'listQuery',
      options: ({ typeId }) => ({
