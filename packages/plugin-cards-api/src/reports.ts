...
const chartTemplates = [
  {
    // progress
    templateType: 'AverageTimeToRespond',
  ...
  },
  {
    ...
    getChartResult: async () => {
      const calculateAverageTimeToClose = tickets => {
        ...
        // Calculate the average time to close in hours
        // const averageTimeToCloseHours =
        //   timeToCloseInHoursArray.reduce((sum, time) => sum + time, 0) /
        //   timeToCloseInHoursArray.length;

        // console.log(
        //   `Average time to close tickets: ${averageTimeToCloseHours.toFixed(
        //     2
        //   )} hours`
        // );
      ...
      const ticketNames = ticket.map((result: any) => result.name);
      const label =
        'View the average amount of time it takes your reps to close tickets. See how this tracks over time.';

      const datasets = [{ label, data: ticketData, labels: ticketNames }];
...
function getTicketCounts(
  ...
  const ticketsCurrentPeriod = tickets.filter(
    ticket => ticket.createdAt >= startDate && ticket.createdAt <= endDate
  );

  const ticketsPreviousPeriod = tickets.filter(
    ticket =>
      ticket.createdAt >= previousStartDate &&
      ticket.createdAt <= previousEndDate
  );

  const totalTicketsCurrentPeriod = ticketsCurrentPeriod.length;
  const totalTicketsPreviousPeriod = ticketsPreviousPeriod.length;

  return {
    totalTicketsCurrentPeriod,
    totalTicketsPreviousPeriod
  };
}
...
//     // Increment counts for each label
//     labels.forEach((label) => {
//       ticketTotals[label] = (ticketTotals[label] || 0) + 1;
//     });

//     // Increment counts for each priority
//     ticketTotals[priority] = (ticketTotals[priority] || 0) + 1;

//     // Increment counts for each tag
//     tags.forEach((tag) => {
//       ticketTotals[tag] = (ticketTotals[tag] || 0) + 1;
//     });
