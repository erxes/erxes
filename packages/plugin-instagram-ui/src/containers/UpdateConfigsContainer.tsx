...
const ConfigsContainer = (props: FinalProps) => {
  ...
  const configs = getConfigsQuery.instagramGetConfigs;
  const configsMap = {};
...
export default withProps<Props>(
  compose(
    graphql(gql(queries.instagramGetConfigs), {
      name: 'getConfigsQuery',
      options: () => ({
        fetchPolicy: 'network-only',
        variables: { kind: 'instagram' }
      })
    }),

    graphql(gql(mutations.instagramUpdateConfigs), {
