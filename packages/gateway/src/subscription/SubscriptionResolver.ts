...
function fieldPathsAsMapFromResolveInfo(resolveInfo: FieldsByTypeName | ResolveTree) {
...
function buildNonPayloadSelections(payload, info): { selections: string, resolveInfo: ResolveTree | FieldsByTypeName } {
...
export default class SubscriptionResolver {
    ...
    public async queryAndMergeMissingData({
        ...
        const { selections, resolveInfo } = buildNonPayloadSelections(payload, info);

        const payloadData = typeof resolveInfo?.name === 'string' ? payload[resolveInfo.name] : Object.values(payload)[0];
