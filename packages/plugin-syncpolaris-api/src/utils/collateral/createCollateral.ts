...
import { openCollateral } from './openCollateral';
  ...
  export const createCollateral = async (subdomain: string, loan:any) => {
    ...
    const customerData = await customFieldToObject(subdomain, 'contacts:customer', customer);
    ...
    if (typeof collateralRes === 'string') {
      ...
      await integrateCollateralToLoan(subdomain,{code:res.acntCode,amount:collateral.marginAmount,loanNumber:loan.number})
    }
  };
