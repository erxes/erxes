...
const GeneralSettings = (props: Props) => {
  const [currentMap, setCurrentMap] = useState({
    ...
    customerDocumentsTxt:
      props.contractType.config?.customerDocuments?.join('\n'),
    companyDocumentsTxt:
      props.contractType.config?.companyDocuments?.join('\n'),
  });
    ...
    <ScrollWrapper>
      <ContentBox>
        ...
        <CollapseContent title={__('Range config')}>
          {renderItem(
            'minInterest',
            `Min interest (month: ${((currentMap.minInterest ?? 0) / 12).toLocaleString()})`,
            {
              type: 'number',
            }
          )}
          {renderItem(
            'maxInterest',
            `Max  interest (month: ${((currentMap.maxInterest ?? 0) / 12).toLocaleString()})`,
            {
              type: 'number',
            }
          )}
        ...
        <CollapseContent title={__('Internet bank')}>
          <FormGroup>
            ...
            <SelectProducts
              ...
              onSelect={(productId) =>
                setState({ ...state, productId: productId as string })
              }
              multi={false}
          ...
          <FormGroup>
            <ControlLabel required={true}>{__('Product Type')}</ControlLabel>
            <FormControl
              name="productType"
              componentclass="select"
              value={state.productType}
              onChange={(e) =>
                setState({ ...state, productType: (e.target as any).value })
              }
            >
          ...
          <FormGroup>
            ...
            <BoardSelectContainer
              ...
              onChangeBoard={(boardId) =>
                setCurrentMap({ ...currentMap, boardId: boardId })
              }
              onChangePipeline={(pipelineId) =>
                setCurrentMap({ ...currentMap, pipelineId: pipelineId })
              }
              onChangeStage={(stageId) =>
                setCurrentMap({ ...currentMap, stageId: stageId })
              }
            />
          </FormGroup>
          <FormGroup>
            <ControlLabel>{__('DAN Rule')}</ControlLabel>
            <FormControl
              name="danRule"
              componentclass="select"
              value={currentMap.danRule}
              onChange={(e) =>
                setCurrentMap({
                  ...currentMap,
                  danRule: (e.target as any).value,
                })
              }
            >
              {['userChoice', 'required', 'allowSkip'].map((typeName) => (
                <option key={typeName} value={typeName}>
                  {typeName}
                </option>
              ))}
        ...
        <CollapseContent title={__('Requirements')}>
          ...
          {renderItem('customerDocumentsTxt', '', {
            componentclass: 'textarea',
          })}
          {renderItem('companyDocumentsTxt', '', {
            componentclass: 'textarea',
          })}
        </CollapseContent>
        <CollapseContent title={__("Requirements")}>
