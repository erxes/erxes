...
function List(props: Props) {
  ...
  const renderDemoAction = (template: ITemplateDoc) => {
    ...
    const url = `${REACT_APP_API_URL}/pl:digitalIncomeRoom/demo/${template._id}`;
  ...
  const renderUseAction = template => {
    ...
    const site = localStorage.getItem('digitalIncomeRoomSiteId') || '';
  ...
  const actionBarLeft = (
    <HeaderContent>
      <Title>{__('Create new room')}</Title>
      <p>
        {__('You can easily customize any of our Portfolio room templates')}
      </p>
    </HeaderContent>
  );
  ...
  return (
    <Wrapper
      header={
        <Wrapper.Header
          title={__('digital Income Rooms')}
          breadcrumb={[
            { title: 'digital Income Rooms', link: '/digitalIncomeRooms' },
            { title: __('New room') }
          ]}
        />
      }
