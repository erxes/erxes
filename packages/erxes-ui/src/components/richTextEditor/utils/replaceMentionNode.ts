...
export function replaceMentionsWithText(doc: JSONContent) {
    function replaceMentions(node: JSONContent) {
        if (node.type === 'mention') {
            ...
            const filteredMarks = node.marks.filter(mark => mark.type === 'spanMark' && mark?.attrs?.['data-type'] === 'mention');
            if (filteredMarks.length > 0) {
                // If the text node has spanMark with type mention, replace it with text
                return { type: 'text', text: (node.marks.find((mark) => mark.type === 'spanMark' && mark?.attrs?.['data-type'] === 'mention')?.attrs?.['data-label']) || node.text };
            }
        } else if (node.content) {
            // Recursively traverse through the content
            return {
