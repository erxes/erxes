...
const spinsMutations = {
  ...
  async spinCampaignsEdit(_root, { _id, ...doc }: ISpinCampaign & { _id: string }, { models }: IContext) {
    return models.SpinCampaigns.updateSpinCampaign(_id, doc)
  },

  async spinCampaignsRemove(_root, { _ids }: { _ids: string[] }, { models }: IContext) {
    return models.SpinCampaigns.removeSpinCampaigns(_ids)
  },
};

checkPermission(spinsMutations, 'spinCampaignsAdd', 'manageSpin');
checkPermission(spinsMutations, 'spinCampaignsEdit', 'manageSpin');
checkPermission(spinsMutations, 'spinCampaignsRemove', 'manageSpin');
