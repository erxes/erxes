...
const TypesListContainer = (props: FinalProps) => {
  const { listMobinetTypeQuery, typesEdit, typesRemove, history } = props;

  if (listMobinetTypeQuery.loading) {
    return <Spinner />;
  }
  ...
  const renderButton = ({
    ...
    return (
      <ButtonMutate
        ...
        refetchQueries={['listMobinetTypeQuery']}
      />
    );
  };
  ...
  const updatedProps = {
    ...
    loading: listMobinetTypeQuery.loading,
    remove,
    renderButton
  };
