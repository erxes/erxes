import FormGroup from '@erxes/ui/src/components/form/Group';
import ControlLabel from '@erxes/ui/src/components/form/Label';
import { IOption } from '@erxes/ui/src/types';
import { __ } from '@erxes/ui/src/utils';
import React from 'react';
import Select from 'react-select-plus';
import { LeftContent, Row } from '../../../styles';

import { ICoordinates } from '../../../types';
import { ICity } from '../types';

type Props = {
  cities: ICity[];
  isRequired?: boolean;
  description?: string;
  defaultValue?: string[] | string;
  multi?: boolean;
  onChange: (value, center?: ICoordinates) => void;
};

class SelectCity extends React.Component<Props, {}> {
  generateOptions(array: ICity[] = []): IOption[] {
    return array.map(item => {
      const city = item || ({} as ICity);

      return {
        value: city._id,
        label: city.name
      };
    });
  }

  onChangeCity = values => {
    const { onChange, multi } = this.props;

    if (!multi) {
      const city =
        values && this.props.cities.find(d => d._id === values.value);

      return onChange(values ? values.value : '', city && city.center);
    }

    onChange(values.map(item => item.value) || []);
  };

  render() {
    const { cities, defaultValue } = this.props;

    return (
      <FormGroup>
        <ControlLabel required={this.props.isRequired}>Cities</ControlLabel>
        <p> {this.props.description ? this.props.description : ''}</p>
        <Row>
          <LeftContent>
            <Select
              placeholder={__('Select city')}
              value={defaultValue}
              onChange={this.onChangeCity}
              options={this.generateOptions(cities)}
              multi={this.props.multi}
            />
          </LeftContent>
        </Row>
      </FormGroup>
    );
  }
}

export default SelectCity;
