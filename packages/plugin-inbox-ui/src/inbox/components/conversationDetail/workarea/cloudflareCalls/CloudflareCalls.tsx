...
const CloudflareCall: React.FC<Props> = ({ conversation, currentUser }) => {
  ...
  const {
    ...
    recordUrl,
    customerPhone,
  } = conversation.cloudflareCallsHistory || ({} as ICallHistory);
...
const WithConsumer = (props: { conversation: IConversation }) => {
  return (
    <AppConsumer>
      {({ currentUser }) => (
        <CloudflareCall {...props} currentUser={currentUser || ({} as IUser)} />
      )}
    </AppConsumer>
  );
};
