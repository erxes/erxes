...
type FinalProps = {
  botsQueryResponse: { integrations: any[] } & QueryResponse;
} & Props;
...
function BotSelectorContainer({ botsQueryResponse, ...props }: FinalProps) {
  ...
  const filteredIntegrations =
    integrations?.filter(
      (integration) => integration.messengerData?.botCheck === true
    ) || [];
  const updatedProps = {
    ...props,
    bots: filteredIntegrations || []
  };
...
export default withProps<Props>(
  compose(
    graphql<Props>(gql(integrations), {
