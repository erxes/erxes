...
class EditFormContainer extends React.Component<FinalProps> {
  ...
  componentDidMount() {
    ...
    this.unsubcribe = detailQuery.subscribeToMore({
      document: gql(subscriptions.growthhacksPipelinesChanged),
      variables: { _id: itemId },
      updateQuery: (
        prev,
        {
          subscriptionData: {
            data: { growthhacksPipelinesChanged },
          },
        }
      ) => {
        if (!growthhacksPipelinesChanged || !growthhacksPipelinesChanged.data) {
          return;
        }

        const { proccessId } = growthhacksPipelinesChanged;
