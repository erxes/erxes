...
const Recipients = (props: Props) => {
  ...
  const extractNameFromEmail = mail => {
    return mail.slice(0, mail.indexOf('@')).replace(/[^a-zA-Z\s ]/g, ' ');
  };

  const renderActions = (contact: any) => {
  ...
  const renderContactDetailBox = (contact: any) => {
    const { fullName, primaryEmail, _id } = contact;
    return (
      <RecipientsDetailBox>
        <DetailSection>
          <NameCard.Avatar company={contact} size={62} />
          <ul>
            <span>
              {!fullName || fullName === '  '
                ? primaryEmail && extractNameFromEmail(primaryEmail)
                : fullName}
            </span>
            <a>{primaryEmail}</a>
          </ul>
        ...
        <DetailActions>
          ...
          <Actions>{renderActions(contact)}</Actions>
        </DetailActions>
      </RecipientsDetailBox>
    );
  };

  const renderRecipients = (contact: any, index: number) => {
    ...
    return (
      <Recipient key={index}>
        ...
        />
        {renderContactDetailBox(contact)}
      </Recipient>
    );
  };

  return (
    <>{collection.map((contact, index) => renderRecipients(contact, index))}</>
  );
};
