...
const generateItemFilter = async (commonQuerySelector, params) => {
...
const productQueries = {
  ...
  async items(
    ...
    const filter = await generateItemFilter(commonQuerySelector, params);

    return paginate(models.Items.find(filter), params);
  },
  ...
  async itemsTotalCount(
    ...
    return models.Items.find(filter).count();
  },

  itemsDetail(_root, { _id }: { _id: string }, { models }: IContext) {
    return models.Items.findOne({ _id }).lean();
  },
