...
import { Props, calendarLink } from "./BoardChooser";

export class BoardChooser extends React.Component<Props> {
  ...
  renderBoards() {
    ...
    return boards.map((board) => {
      ...
      return (
        <li
          key={board._id}
          className={`${currentBoard && board._id === currentBoard._id && "active"}`}
        >
  ...
  renderGroups() {
    ...
    if (groups.length === 0 || !currentBoard) {
      return (
        <EmptyState
          ...
          size="small"
          extra={<Button btnStyle="warning" size="small">
            <Link
              to={`/settings/calendars?boardId=${currentBoard ? currentBoard._id : ""}`}
            >
              Create Group
            </Link>
          </Button>} />
    ...
    return groups.map((group) => {
      return (
        <li
          key={group._id}
          className={`${currentGroup && group._id === currentGroup._id && "active"}`}
        >
          <Link
  ...
  render() {
    ...
    return (
      <Controls>
        <Dropdown
          as={DropdownToggle}
          toggleComponent={<HeaderButton $hasBackground={true}>
