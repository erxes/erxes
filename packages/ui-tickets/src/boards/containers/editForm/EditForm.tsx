...
class EditFormContainer extends React.Component<FinalProps> {
  ...
  componentDidMount() {
    ...
    this.unsubcribe = detailQuery.subscribeToMore({
      document: gql(subscriptions.ticketPipelinesChanged),
      variables: { _id: itemId },
      updateQuery: (
        prev,
        {
          subscriptionData: {
            data: { ticketPipelinesChanged },
          },
        }
      ) => {
        if (!ticketPipelinesChanged || !ticketPipelinesChanged.data) {
          return;
        }

        const { proccessId } = ticketPipelinesChanged;
