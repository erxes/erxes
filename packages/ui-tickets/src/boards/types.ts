...
export interface IConversionStagePurchase extends IStage {
  initialTicketsTotalCount: number;
  inProcessTicketsTotalCount: number;
  stayedTicketsTotalCount: number;
...
export type BoardCountsQueryResponse = {
  ticketsBoardCounts: IBoardCount[];
} & QueryResponse;

export type PipelinesQueryResponse = {
  ticketsPipelines: IPipeline[];
  loading: boolean;
  refetch: ({
    boardId,
    type
...
export type PipelineLabelsQueryResponse = {
  ticketsPipelineLabels: IPipelineLabel[];
} & QueryResponse;

export type PipelineLabelDetailQueryResponse = {
  ticketsPipelineLabelDetail: IPipelineLabel;
} & QueryResponse;
