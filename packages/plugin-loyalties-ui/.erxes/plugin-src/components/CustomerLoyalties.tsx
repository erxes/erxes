import { Table, DataWithLoader } from "@erxes/ui/src/components";
import { __, formatValue, renderFullName } from "@erxes/ui/src/utils";
  ...
  renderTable(loyalties) {
    return (
      <Table whiteSpace="wrap" hover={true} bordered={true} condensed={true}>
        ...
        <tbody id="loyalties">
          {loyalties.map((loyalty) => this.renderRow(loyalty))}
        </tbody>
      </Table>
    );
  }
  ...
  render() {
    ...
    const breadcrumb = [
      { title: __("Contacts"), link: "/contacts" },
      {
        ...
        link: `/contacts/details/${loyalty.customerId}`,
      },
    ...
    return (
      <Wrapper
        header={<Wrapper.Header breadcrumb={breadcrumb} title="Loyalty" />}
