...
const TypesListContainer = (props: FinalProps) => {
  const { listGoalsTypeQuery, typesEdit, typesRemove, history } = props;

  if (listGoalsTypeQuery.loading) {
    return <Spinner />;
  }
  ...
  const renderButton = ({
    ...
    return (
      <ButtonMutate
        ...
        refetchQueries={['listGoalsTypeQuery']}
      />
    );
  };
  ...
  const updatedProps = {
    ...
    loading: listGoalsTypeQuery.loading,
    remove,
    renderButton
  };
