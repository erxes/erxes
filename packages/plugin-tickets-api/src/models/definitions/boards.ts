...
export const pipelineSchema = new Schema({
  ...
  isCheckUser: field({
    type: Boolean,
    optional: true,
    label: 'Show only the users created or assigned tickets',
  }),
  isCheckDepartment: field({
    type: Boolean,
    optional: true,
    label: 'Show only the departments created or assigned tickets',
  }),
  excludeCheckUserIds: field({
    type: [String],
    optional: true,
    label: 'Users elligible to see all tickets',
  }),
