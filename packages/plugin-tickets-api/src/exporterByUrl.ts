...
const fillTicketProductValue = async (
  subdomain,
  column,
  item,
  sheet,
  ...
  if (productsData.length === 0) {
    ...
    addCell(column, "-", sheet, columnNames, ticketRowIndex);

    return { rowIndex, ticketRowIndex };
  }

  if (ticketIds.length === 0) {
    ticketIds.push(item._id);
  } else if (!ticketIds.includes(item._id)) {
    ticketIds.push(item._id);
    rowIndex = ticketRowIndex;
  }

  ticketRowIndex = rowIndex;
  ...
  for (const productData of productsData) {
    ...
    if (cellValue) {
      addCell(column, cellValue, sheet, columnNames, ticketRowIndex);
  ...
  for (const item of data) {
    ...
    for (const column of headers) {
      if (column.name.startsWith("customFieldsData")) {
        ...
        const indexes = await fillTicketProductValue(
          subdomain,
          column,
          item,
          sheet,
        ...
        ticketRowIndex = indexes?.ticketRowIndex;
      } else {
        let index = rowIndex;
        if (type === MODULE_NAMES.TICKET) {
          index = ticketRowIndex === 0 ? rowIndex : ticketRowIndex;
        }
