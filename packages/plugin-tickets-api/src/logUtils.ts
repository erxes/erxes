...
type BoardItemDocument = ITicketDocument;
...
const gatherTicketFieldNames = async (
  models: IModels,
  subdomain: string,
  doc: ITicketDocument,
  prevList?: LogDesc[]
...
const gatherDescriptions = async (
  ...
  switch (type) {
    ...
    case MODULE_NAMES.TICKET:
      description = `"${object.name}" has been ${action}d`;
      extraDesc = await gatherTicketFieldNames(models, subdomain, object);

      if (updatedDocument) {
        extraDesc = await gatherTicketFieldNames(
          models,
          subdomain,
          updatedDocument,
          extraDesc
