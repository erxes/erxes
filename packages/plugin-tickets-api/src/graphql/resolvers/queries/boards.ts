...
const boardQueries = {
  ...
  async ticketsPipelineStateCount(
    _root,
   ...
   */
  async ticketsPipelineAssignedUsers(
    _root,
    { _id }: { _id: string },
    { models }: IContext
  ) {
  ...
  async ticketsBoardContentTypeDetail(_root, args, { subdomain }: IContext) {
    return getContentTypeDetail(subdomain, args);
  },

  async ticketsBoardLogs(_root, args, { subdomain, models }: IContext) {
