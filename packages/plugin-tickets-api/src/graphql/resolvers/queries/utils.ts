    ...
    if (
      ...
      if (
        ...
        if (pipeline.isCheckBranch) {
          ...
          const otherBranchUsers = await sendCoreMessage({
            subdomain,
            action: "users.find",
            data: {
              query: { branchIds: { $in: commonIds } },
            },
            isRPC: true,
            defaultValue: [],
          });

          for (const branchUser of otherBranchUsers) {
            includeCheckUserIds = [...includeCheckUserIds, branchUser._id];
          }

          if (
            !!pipelineBranchIds.filter((departmentId) =>
              userBranchIds.includes(departmentId)
            ).length
          ) {
            includeCheckUserIds = includeCheckUserIds.concat(user._id || []);
          }
