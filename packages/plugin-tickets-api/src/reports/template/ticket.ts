...
export const ticketCharts = [
  // TicketCustomProperties
  {
    ...
    filterTypes: [
      ...
      // TAG FILTER
      {
        ...
        fieldQueryVariables: `{"type": "ticket:tickets", "perPage": 1000}`,
        multi: true,
      ...
      // LABEL FILTER
      {
        fieldName: "labelIds",
        fieldType: "select",
        fieldQuery: "ticketsPipelineLabels",
        multi: true,
        fieldValueVariable: "_id",
        fieldLabelVariable: "name",
        logics: [
      ...
      // CUSTOM PROPERTIES FILTER
      {
        ...
        fieldValueVariable: "_id",
        fieldLabelVariable: "name",
        fieldQueryVariables: `{"contentType": "ticket:tickets"}`,
        multi: true,
      ...
      // CUSTOM PROPERTIES FIELD FILTER
      {
        ...
        fieldQueryVariables: `{"contentType": "ticket:tickets", "isVisible": true}`,
        logics: [
          {
            logicFieldName: "groupIds",
            logicFieldVariable: "groupIds",
            logicFieldExtraVariable: `{"contentType": "ticket:tickets"}`
          }
        ],
        multi: true,
  ...
  // TicketClosedTotalsBySource
  {
    ...
    getChartResult: async (
      ...
      const totalCountBySource = (tickets || []).reduce(
  ...
  // TicketAverageTimeToCloseByRep
  {
    ...
    getChartResult: async (
      ...
      const stageIds = await getStageIds(
        { ...filter, stageProbability: PROBABILITY_CLOSED["ticket"] },
        "ticket",
        models
      );
  ...
  // TicketsTotalCount
  {
    ...
    getChartResult: async (
      ...
      if (chartType === "number") {
        const ticketsCount =
          await models.Tickets.find(matchFilter).countDocuments();
      ...
      return {
        title,
        ...buildData({ chartType, data: tickets, measure, dimension })
      };
    },
    filterTypes: [
      // DIMENSION FILTER
