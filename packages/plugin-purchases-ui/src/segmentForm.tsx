...
class Form extends React.Component<any, any, any> {
  ...
  render() {
    ...
    if (component === 'filter') {
      if (propertyType && !['purchases:purchase'].includes(propertyType)) {
        return null;
      }

      if (!hideDetailForm && ['purchases:purchase'].includes(type)) {
        return null;
      }

      return content;
    } else if (['purchases:purchase'].includes(type)) {
      return content;
    }
  }
}
...
const generateVariable = (type, propertyType) => {
  if (['purchases:purchase'].includes(type)) {
