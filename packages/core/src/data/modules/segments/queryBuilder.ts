...
export const generateQueryBySegment = async (
  ...
  for (const condition of propertyConditions) {
    ...
    if (field && condition.propertyType) {
      ...
      if (contentType === condition.propertyType) {
        ...
        const existAssociationFilters = associationFilters.find(
          ({ propertyType }) => propertyType === condition.propertyType
        );

        if (existAssociationFilters) {
          associationFilters = associationFilters.map(associationFilters => {
            if (associationFilters.propertyType === condition.propertyType) {
              return {
                ...associationFilters,
                positiveQueries: mergeToMust(
                  associationFilters.positiveQueries,
                  positiveQuery
                ),
                negativeQueries: mergeToMust(
                  associationFilters.negativeQueries,
                  negativeQuery
                )
              };
            }
  ...
  if (associationFilters.length) {
    for (const {
      ...
      positiveQueries,
      negativeQueries
    } of associationFilters) {
      const ids = await associationPropertyFilter(subdomain, {
        serviceName,
        mainType: contentType,
        propertyType,
        positiveQuery: positiveQueries,
        negativeQuery: negativeQueries
      });
