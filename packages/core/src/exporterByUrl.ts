...
} from "./data/modules/coc/companies";
import {
  Builder as CustomerBuildQuery,
  IListArgs as ICustomerListArgs
} from "./data/modules/coc/customers";
...
const fillLeadHeaders = async (models: IModels, formId: string) => {
...
const buildLeadFile = async (
  ...
  const headers: IColumnLabel[] = await fillLeadHeaders(models, formId);
...
export const buildFile = async (
  ...
  if (type === MODULE_NAMES.CUSTOMER && query.form && query.popupData) {
    await buildLeadFile(models, data, query.form, sheet, columnNames, rowIndex);
      ...
      for (const column of headers) {
        if (column.name.startsWith("customFieldsData")) {
          const { field, value } = await getCustomFieldsData(
            selector => models.Fields.findOne(selector),
            item,
            column,
            type
          );
