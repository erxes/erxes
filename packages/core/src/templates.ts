...
export default {
  ...
  useTemplate: async ({ subdomain, data }) => {
    ...
    switch (contentType) {
      case 'productCategories':
        reDirect = await productCategoryUseTemplate({ models, content, relatedContents })
        break;
      case 'forms':
        reDirect = await formUseTemplate({ models, content, relatedContents, currentUser })
        break;
    }
  ...
  getRelatedContent: async ({ subdomain, data }) => {
    ...
    if (serviceName === 'core') {
      ...
      switch (contentType) {
        case 'productCategories':
          relatedContents = await productRelatedContents({ models, mainContent })
          break;
        case 'forms':
...
export const formRelatedContents = async ({ models, mainContent }) => {
  ...
  const fields = await models.Fields.find({ contentTypeId: formId, contentType: 'form', isDefinedByErxes: false }, { _id: 0, scopeBrandIds: 0, lastUpdatedUserId: 0, productCategoryId: 0 }).lean()
...
export const productCategoryUseTemplate = async ({ models, content, relatedContents }) => {
...
export const formUseTemplate = async ({ models, content, relatedContents, currentUser }) => {
