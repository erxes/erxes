...
const getCustomer = async (subdomain: string, customerId: string, customerType: string) => {
  let customer;
...
export const getMsdCustomerInfo = async (subdomain: string, models: IModels, customerId: string, customerType: string, brandId: string, config) => {
  ...
  let filterStr;
  const now = new Date();
  const relation = await models.CustomerRelations.findOne({ customerId, brandId }).lean();

  const customer = await getCustomer(subdomain, customerId, customerType)

  if (relation) {
    const dayBefore = Math.round((now.getTime() - relation.modifiedAt.getTime()) / (1000 * 3600 * 24));
  ...
  return { relation: await models.CustomerRelations.findOne({ customerId, brandId }).lean(), customer };
}
...
export const customerToDynamic = async (subdomain, params, models) => {
  ...
  for (const brandId of brandIds) {
    ...
    await getMsdCustomerInfo(subdomain, models, customer._id, 'customer', brandId, config)
  }
};
