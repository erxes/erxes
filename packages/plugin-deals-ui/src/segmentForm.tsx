...
import { queries } from '@erxes/ui-deals/src/settings/boards/graphql';
...
class Form extends React.Component<any, any, any> {
  ...
  render() {
    ...
    if (component === 'filter') {
      if (propertyType && !['deals:deal'].includes(propertyType)) {
        return null;
      }

      if (!hideDetailForm && ['deals:deal'].includes(type)) {
        return null;
      }

      return content;
    } else if (['deals:deal'].includes(type)) {
      return content;
    }
  }
}
