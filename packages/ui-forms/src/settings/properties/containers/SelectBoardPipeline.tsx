...
import { mutations } from '@erxes/ui-tickets/src/settings/boards/graphql';
import { queries as ticketQueries } from '@erxes/ui-tickets/src/boards/graphql';
...
const getRefetchQueries = () => {
  return [
    {
      query: gql(ticketQueries.boards),
      variables: {},
    },
  ];
};
...
export default withProps<Props>(
  compose(
    graphql<Props, BoardsQueryResponse, { type: string }>(
      gql(ticketQueries.boards),
      {
        name: 'boardsQuery',
        options: ({ type }) => ({
          variables: {
