...
export class ViberAPI {
  ...
  constructor(config: any) {
    ...
    this.headers = {
      'X-dailyco-Auth-Token': config.token
    };
  }
  ...
  async registerWebhook(): Promise<any> {
    ...
    const url: string = `${domain}/pl:dailyco/webhook/${this.integrationId}`;

    const payload: IRequestParams = {
      method: 'POST',
      headers: this.headers,
      url: 'https://chatapi.dailyco.com/pa/set_webhook',
      body: {
        url,
        event_types: [
          'delivered',
  ...
  async sendMessage(message: IMessage): Promise<any> {
    ...
    const commonReqestParams = {
      method: 'POST',
      headers: this.headers,
      url: 'https://chatapi.dailyco.com/pa/send_message'
    };
