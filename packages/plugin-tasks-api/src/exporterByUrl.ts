...
const fillTaskProductValue = async (
  subdomain,
  column,
  item,
  sheet,
  ...
  if (productsData.length === 0) {
    ...
    addCell(column, "-", sheet, columnNames, taskRowIndex);

    return { rowIndex, taskRowIndex };
  }

  if (taskIds.length === 0) {
    taskIds.push(item._id);
  } else if (!taskIds.includes(item._id)) {
    taskIds.push(item._id);
  ...
  for (const productData of productsData) {
    ...
    if (cellValue) {
      addCell(column, cellValue, sheet, columnNames, taskRowIndex);
  ...
  for (const item of data) {
    ...
    for (const column of headers) {
      if (column.name.startsWith("customFieldsData")) {
        ...
        const indexes = await fillTaskProductValue(
          subdomain,
          column,
          item,
          sheet,
        ...
        taskRowIndex = indexes?.taskRowIndex;
      } else {
        let index = rowIndex;
        if (type === MODULE_NAMES.TICKET) {
          index = taskRowIndex === 0 ? rowIndex : taskRowIndex;
        }
