...
const boardQueries = {
   ...
   */
  async tasksPipelineAssignedUsers(
    _root,
    { _id }: { _id: string },
    { models }: IContext
  ) {
  ...
  async tasksItemsCountByAssignedUser(
    _root,
    {
      pipelineId,
      type,
  ...
  async tasksArchivedStagesCount(
    _root,
   ...
   *  ConvertTo info
   */
  async tasksConvertToInfo(
    _root,
    { conversationId }: { conversationId: string },
    { models: { Stages, Pipelines, Boards, Tasks } }: IContext
  ) {
  ...
  async taskItemsCountBySegments(
    _root,
    {
      type,
      boardId,
  ...
  async tasksBoardContentTypeDetail(_root, args, { subdomain }: IContext) {
    return getContentTypeDetail(subdomain, args);
  },

  async tasksBoardLogs(_root, args, { subdomain, models }: IContext) {
