...
type BoardItemDocument = ITaskDocument;
...
const gatherTaskFieldNames = async (
  models: IModels,
  subdomain: string,
  doc: ITaskDocument,
  prevList?: LogDesc[]
...
const gatherDescriptions = async (
  ...
  switch (type) {
    ...
    case MODULE_NAMES.TICKET:
      description = `"${object.name}" has been ${action}d`;
      extraDesc = await gatherTaskFieldNames(models, subdomain, object);

      if (updatedDocument) {
        extraDesc = await gatherTaskFieldNames(
          models,
          subdomain,
          updatedDocument,
          extraDesc
...
export const putDeleteLog = async (
  ...
  await commonPutDeleteLog(
    subdomain,
    { ...logDoc, description, extraDesc, type: `tasks:${logDoc.type}` },
    user
  );
};
...
export const putActivityLog = async (
  ...
  const updatedParams = {
    ...params,
    data: { ...data, contentType: `tasks:${data.contentType}` }
  };
