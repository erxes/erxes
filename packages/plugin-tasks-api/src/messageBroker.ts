...
export const setupMessageConsumers = async () => {
  consumeRPCQueue('tasks:create', async ({ subdomain, data }) => {
  ...
  consumeRPCQueue('tasks:remove', async ({ subdomain, data: { _ids } }) => {
    const models = await generateModels(subdomain);
  ...
  consumeRPCQueue('tasks:find', async ({ subdomain, data }) => {
    const models = await generateModels(subdomain);
  ...
  consumeRPCQueue('tasks:findOne', async ({ subdomain, data }) => {
    const models = await generateModels(subdomain);
  ...
  consumeQueue(
    'tasks:taskPipelinesChanged',
    async ({ subdomain, data: { pipelineId, action, data } }) => {
      graphqlPubsub.publish('taskPipelinesChanged', {
