...
export const buildPipeline = (filter: any, matchFilter: any) => {
    ...
    const { dimension: dimensions, measure: measures, frequencyType = '%m' } = filter

    const pipeline: any = [];

    const matchStage = {}

    if ((dimensions || []).some(dimension => ['brand', 'channel', 'integration'].includes(dimension))) {
...
export const formatData = (data, frequencyType) => {
    ...
    formattedData.map(item => {
        ...
        if (item.hasOwnProperty('teamMember')) {
            ...
            item['teamMember'] = user.details?.fullName || `${user.details?.firstName} ${user.details?.lastName}` || user.email
        }
...
export const buildData = ({ chartType, data, measure, dimension, frequencyType }) => {
