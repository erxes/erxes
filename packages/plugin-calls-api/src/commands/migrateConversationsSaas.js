...
const { CORE_MONGO_URL, MONGO_URL, NODE_ENV } = process.env;
...
const command = async () => {
    ...
    for (const org of orgs) {
        try {
            ...
            let client;
            if (NODE_ENV === 'dev') {
                tempMongoUrl = `mongodb://127.0.0.1:27017/erxes_${org._id.toString()}?directConnection=true`
                client = new MongoClient(tempMongoUrl);
            } else {
                client = new MongoClient(
                    MONGO_URL.replace('<organizationId>', org._id.toString())
                );
            }
            ...
            try {
                ...
                console.log(`Saas process finished at: ${new Date()}`);
...
const getOrCreateCustomer = async (callAccount) => {
    ...
    if (!callCustomer) {
        try {
            ...
            callCustomer = await CallCustomers.findOne({
                primaryPhone: primaryPhone,
            });
        } catch (e) {
            throw new Error(e);
