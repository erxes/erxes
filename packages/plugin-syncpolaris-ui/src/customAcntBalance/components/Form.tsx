...
class FormComponent extends React.Component<Props & ICommonFormProps, State> {
  constructor(props: Props) {
    ...
    const { syncpolaris } = this.props;

    this.state = {
      expiryDate: syncpolaris
        ? syncpolaris.expiryDate
  ...
  generateDoc = (values: { _id?: string; name: string; content: string }) => {
    ...
    if (syncpolaris) {
      finalValues._id = syncpolaris._id;
    }
  ...
  renderContent = (formProps: IFormProps) => {
    ...
    const { syncpolaris, types, afterSave, closeModal, renderButton } =
      this.props;
    const { values, isSubmitted } = formProps;
    const object = syncpolaris || ({} as ISyncpolaris);
    return (
      <>
        <FormGroup>
