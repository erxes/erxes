...
class BoardSelectContainer extends React.Component<FinalProps> {
  onChangeBoard = (boardId: string) => {
    ...
    this.props.pipelinesQuery
      .refetch({ boardId })
      .then(({ data }) => {
        const pipelines = data.tasksPipelines;

        if (pipelines.length > 0 && !this.props?.autoSelectPipeline) {
  ...
  render() {
    ...
    const pipelines = pipelinesQuery.tasksPipelines || [];
    const stages = stagesQuery.tasksStages || [];
