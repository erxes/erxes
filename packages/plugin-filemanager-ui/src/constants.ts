  ...
  steps: [
    {
      ...
      description:
        `<ul><li>${__("Make sure you’ve created your Brands")}</li><li>${__("Click on “Add Knowledgebase” to create one for a specific Brand")}</li><li>${__("Click on the “Settings” button and “Add Categories”. A good one to get started with would be “General, Pricing, etc.”")}</li><li>${__("Click on “Add Articles” to start adding help articles")}</li></ul>`,
      html: true
    },
    {
      title: __('Install the script'),
      description:
        `<ul><li>${__("Copy the individual script by clicking on the Settings button.")}</li><li>Use <a href='/settings/scripts'>Script Manager</a> to avoid script duplication errors if you’re planning to display this popup along with any other erxes widgets</li></ul>`,
      html: true,
