...
export interface IPutResponseDocument extends Document {
  data: Date;
  orderId: string;
  hasVat: boolean;
  hasCitytax: boolean;
...
export const unitedCodesSchema = schemaHooksWrapper(
  new Schema({
    _id: field({ pkey: true }),
    modifiedAt: field({ type: Date, label: "Modified at" }),
    categoryId: field({ type: String, label: "Product Category" }),
    unitedCode: field({ type: String, label: "United Code" }),
  }),
