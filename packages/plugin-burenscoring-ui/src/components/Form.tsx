...
type Props = {
  ...
  burenscoring?: IBurenscoring;
  burenscorings?: IBurenscoring[];
  types?: IType[];
} & ICommonFormProps;
...
class FormComponent extends React.Component<Props & ICommonFormProps, State> {
  constructor(props: Props) {
    ...
    this.state = {
      expiryDate: burenscoring
        ? burenscoring.expiryDate
        : dayjs()
            .add(30, 'day')
            .toDate()
    };
  ...
  generateDoc = (values: { _id?: string; name: string; content: string }) => {
    ...
    if (burenscoring) {
      finalValues._id = burenscoring._id;
    }
  ...
  renderContent = (formProps: IFormProps) => {
    ...
    const { burenscoring, types, afterSave, closeModal, renderButton } = this.props;
    const { values, isSubmitted } = formProps;
    const object = burenscoring || ({} as IBurenscoring);
    return (
      <>
        <FormGroup>
