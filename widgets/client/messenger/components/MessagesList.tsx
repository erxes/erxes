...
import { IIntegrationMessengerData, IIntegrationMessengerDataMessagesItem, IIntegrationUiOptions } from '../../types';
...
type Props = {
  ...
  saveGetNotified: (doc: { type: string; value: string }, callback?: () => void) => void;
  isLoggedIn: () => boolean;
  getColor?: string;
...
class MessagesList extends React.Component<Props, State> {
  ...
  componentDidMount() {
    const { messengerData, messages, conversationId = '', getBotInitialMessage } = this.props;

    const newConversation = messages.length === 0 && (!conversationId || (conversationId || '').length === 0);
  ...
  componentWillUpdate() {
    ...
    if (node) {
      this.shouldScrollBottom = node.scrollHeight - (node.scrollTop + node.offsetHeight) < 20;
    }
  }
  ...
  onNotify = ({ type, value }: { type: string; value: string }) => {
    this.props.saveGetNotified({ type, value }, () => {
      this.setState({ hideNotifyInput: true }, () => setLocalStorageItem('hasNotified', 'true'));
    });
  };
  ...
  renderNotifyInput(messengerData: IIntegrationMessengerData) {
    ...
    return (
      <li className="erxes-spacial-message with-background auth">
        <AccquireInformation save={this.onNotify} color={getColor} loading={false} textColor={uiOptions.textColor || '#fff'} />
      </li>
    );
  }
  ...
  renderSkillOptionsMessage(messengerData: IIntegrationMessengerData) {
    ...
    if (!messengerData.skillData || !newConversation || this.props.selectedSkill) {
      return null;
    }
      ...
      <div className="app-message-box call-request" style={{ borderColor: color }}>
        ...
        <div className="call-buttons">
          <button className="erxes-button" style={{ background: color }} onClick={sendCallRequest}>
            {iconCall(textColor)}
            <span style={{ background: color, color: textColor }}>{__('Audio call')}</span>
          </button>
          <button className="erxes-button" style={{ background: color }} onClick={sendCallRequest}>
            {iconVideo(textColor)}
  ...
  renderSingleMessage = (message: any) => {
    const { replyAutoAnswer, sendTypingInfo, uiOptions, messengerData } = this.props;
    ...
    const content = showBotMessage ? (
      <MessageBot color={color} key={message._id} {...messageProps} scrollBottom={this.scrollBottom} />
    ) : (
    ...
    if (_id < 0) {
      return (
        <RTG.CSSTransition key={message._id} timeout={500} classNames="slide-in">
          {content}
        </RTG.CSSTransition>
      );
    }
  ...
  renderMessages() {
    return <RTG.TransitionGroup component={null}>{this.props.messages.map(this.renderSingleMessage)}</RTG.TransitionGroup>;
  }

  renderBotOperator() {
    const { operatorStatus, conversationId, getColor } = this.props;

    if (!operatorStatus || !conversationId || operatorStatus === OPERATOR_STATUS.OPERATOR) {
      return null;
    }
    ...
    return (
      <div className="bot-message">
        <div className="quick-replies">
          <div className="reply-button" role="button" tabIndex={0} onClick={this.handleOperatorStatus} style={{ borderColor: getColor }}>
  ...
  renderErrorMessage() {
    ...
    return (
      <li className="from-customer">
        <div className="app-message-box spaced flexible" style={{ borderColor: 'red' }}>
