...
erxes gives you many options for installing the erxes platform. Below you will find two guides:
...
- If you are using a **sub-domain**, you will need to follow the instructions of updating the `A Records' of the hosting company for your website.
- If you are NOT using a **sub-domain**, then you will need to follow the instructions of your domain name registrar.
...
- Add a user called "erxes". When prompted, enter in a **unique** password. Finish adding the user simply by pressing enter through the questions.
...
Your last step as **root** user, is to set-up a basic firewall for your server.
...
If you got the above output, then you are ready to continue with installing **Node.js**.
...
With **Node.js** installed, you can continue with installing **PM2**.
...
- Install **libwww-perl** to use for testing **Elasticsearch** is correctly installed. Press `y` when prompted to continue.

```bash
sudo apt install libwww-perl
```

- Verify **Elasticsearch** is installed by testing the `GET /` response
...
<html>
  ...
  <body>
    ...
    <ul>
      ...
      <li><a href="vmlinuz">vmlinuz</a></li>
      <li><a href="vmlinuz.old">vmlinuz.old</a></li>
    </ul>
  </body>
</html>
```

If the `GET /` command outputted the HTML page, then you are ready to configure your **NGINX** proxy server.
...
- Install **NGINX** with the following commands. Press `enter` when prompted to continue.
...
After successfully each of the above packages, you may continue and configure **PM2**.
...
- Copy the following content into the file. **DO NOT SAVE THE FILE UNTIL ALL STEPS BELOW ARE COMPLETE**.
