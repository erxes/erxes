...
The script created a random password for your **MongoDB** database. We need to copy it from the `ecosystem.json` file and set it as an `env` variable. This will require several steps.
...
```bash
nvm use default
node erxes-api/dist/commands/initProject
```
...
node erxes-api/dist/commands/loadPermissionData
```
