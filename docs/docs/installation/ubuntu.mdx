...
- Create a server with **Ubuntu 18.04** installed, a minimum of **4 CPUs** and **8GB RAM**.
...
An **erxes** user was automatically created. Now we need a unique password. This
...
## Final MongoDB Configuration steps

The script created a random password for your **MongoDB** database. We need to copy it from the `ecosystem.config.js` file and set it as an `env` variable.
...
- Copy the MONGO_URL and exit the nano editor
...
(**Note**: Some people get an error, **WARNING: REMOTE HOST IDENTIFICATION HAS CHANGED!**, please see the solution [here](/installation/ubuntu#known-issues-and-frequent-questions).)
...
```bash
curl -L https://github.com/erxes/erxes/releases/download/0.16.0/erxes-0.16.0.tar.gz | tar -xz -C ~/erxes.io/erxes/ \
    && curl -L https://github.com/erxes/erxes/releases/download/0.16.0/erxes-widgets-0.16.0.tar.gz | tar -xz -C ~/erxes.io/erxes-widgets/ \
    && curl -L https://github.com/erxes/erxes-api/releases/download/0.15.5/erxes-api-0.15.5.tar.gz | tar -xz -C ~/erxes.io/erxes-api/ \
    ...
    && curl -L https://github.com/erxes/erxes-integrations/releases/download/0.15.5/erxes-integrations-0.15.5.tar.gz | tar -xz -C ~/erxes.io/erxes-integrations/ \
