...
If you are using a basic server (the above minimum server requirements), you **must use** Option **2 Use elasticsearch.erxes.io** which uses elasticsearch hosted by erxes (free). Option 1 requires a server with 4GB RAM or higher.
