...
This documentation will guide you through the installation of an erxes project using **<a href="https://www.docker.com/" target="_blank">Docker</a>** on **<a href="https://www.apple.com/macos/monterey/" target="_blank">MacOs</a>**.

Docker is an open platform that allows you to develop, ship and run applications by using containers (i.e. packages containing all the parts an application needs to function, such as libraries and dependencies).
...
:::caution
Erxes code takes approximately 12GB storage space. Please ensure you have enough space on your device before proceeding
...
9. Switch the dev branch by using the following command.
...
12. Run the following command to change the folder.
...
:::caution
You are not required to add it under `logs` plugin since it has no ui.
:::
...
16. Run the following command to start your erxes project.
