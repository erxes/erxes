/**
 * The shape this renderer reads off a block. Structural on purpose: the
 * editor's own types live in a frontend package, and nothing here needs them.
 */
...
export interface TBlocksToHtmlConfig {
  colors?: ColorConfig;
  baseFont?: string;
  baseFontSize?: string;
...
const getColor = (
  ...
  config?: TBlocksToHtmlConfig,
): string => {
  const colors: ColorConfig = config?.colors || COLORS_DEFAULT;
...
const stylesToCss = (styles?: Record<string, any>, config?: TBlocksToHtmlConfig): string => {
  if (!styles) return '';
...
const renderInlineContent = (content: any[], config?: TBlocksToHtmlConfig): string => {
...
const getBaseStyles = (config?: TBlocksToHtmlConfig) => {
  return {
...
const resolveSrc = (url: string, config?: TBlocksToHtmlConfig): string =>
  config?.resolveImageUrl ? config.resolveImageUrl(url) : url;

const renderBlock = (block: TEmailBlock, config?: TBlocksToHtmlConfig): string => {
...
export const emailHtmlWrapper = (html: string, config?: TBlocksToHtmlConfig) => {
