...
async function fixTranslationIndex() {
  try {
    ...
    const collections = await db
      .listCollections({ name: 'cms_translations' })
      .toArray();
    if (collections.length === 0) {
      console.log(
        '⚠️  cms_translations collection does not exist yet. Nothing to migrate.',
      );
    ...
    const oldPostIdIndex = indexes.find(
      (idx) => idx.name === 'postId_1_language_1_type_1',
    );
    if (oldPostIdIndex) {
    ...
    if (nullObjectIdDocs > 0) {
      console.log(
        `🧹 Found ${nullObjectIdDocs} documents with null objectId, cleaning up...`,
      );
      ...
      console.log(
        `🗑️  Deleted ${deleteResult.deletedCount} documents with null objectId`,
      );
    }
    ...
    if (documentsWithPostId > 0) {
      console.log(
        `🔄 Found ${documentsWithPostId} documents with postId but no objectId`,
      );
      ...
      console.log(
        `✅ Migrated ${updateResult.modifiedCount} documents from postId to objectId`,
      );
    }

    // Check if the correct objectId-based index exists
    const objectIdIndex = indexes.find(
      (idx) => idx.name === 'objectId_1_language_1_type_1',
    );
    if (!objectIdIndex) {
