...
- Internal Viber helpers verify webhook signatures, fetch and validate bot
  ...
  settings, saves the connection, and awaits webhook registration before
  returning Frontline's success/error response. Setup failures preserve both
  local records when the Viber record exists, with a fixed message directing
  the caller to Repair. The common record is rolled back only when a lookup
  confirms that no Viber connection was saved; recovery lookup failures propagate.
  ...
  It is called by removal and registration during creation or Repair;
  live provider behavior remains unverified. The internal
  ...
  then the `VIBER_MEDIA_ALLOWED_HOSTNAMES` environment override, then the built-in
  `dl-media.viber.com` and `content.cdn.viber.com` defaults. Saving an empty array
  explicitly disables media and never falls through to either fallback.
  Both the receiver and setup readiness use this lookup on each request.
  The defaults are documented incoming-media hosts, not an exhaustive CDN list
  or proof of live bot compatibility.
  `downloadAndStoreViberAttachment`
  composes downloading and tenant storage into native attachment metadata,
  without creating messages or handling replays. Incoming media requires an
  approved source host and a supported tenant storage provider.
...
`__tests__/creation.spec.ts` composes the real creation resolver, Viber adapter,
helpers, callback builder, and HTTP utilities with mocked database and HTTP I/O;
it covers setup ordering, partial failures, rollback, and recovery through Repair.
...
`constants.ts` owns the documented `VIBER_DEFAULT_MEDIA_HOSTNAMES` fallback;
the settings resolver returns a fresh copy without seeding tenant documents.
It defines `ViberMediaType` and the incoming-media size policy shared
...
accountId, brandId, data)` — `channelId` is **nullable** for every kind;
  ...
  `integrationsAdd` before channel access or creation. Viber creation awaits
  webhook registration and the healthy-state save. A setup error may leave both
  records available for Repair: callers must reread connection state instead of
  assuming that every failed mutation rolled back. The GraphQL schema is unchanged.
...
- Viber Bot REST API: internal `POST https://chatapi.viber.com/pa/set_webhook`
  ...
  the webhook. Removal and registration during creation or Repair are wired.
...
- Internal Viber media policy consumes `VIBER_MEDIA_ALLOWED_HOSTNAMES` through
  ...
  list of approved bare hostnames, not URLs or wildcard patterns. Missing or
  blank environment configuration falls back to the built-in hosts through
  `resolveViberMediaSettings`; the receiver consumes that effective policy.
...
- `formatViberText(text)` in `utils/content.ts` rejects blank strings without
  ...
  the formatter itself performs no persistence. The text processor calls it
...
- `viberCreateIntegration` is the creation adapter in `messageBroker.ts`, not
  ...
  `createViberIntegration` followed by `registerViberWebhook` inside
  `withErrorHandling`. Persist the credential record before requesting registration.
  JSON parse failures use a fixed error message without reflecting the settings.
- The common `createOnService` recovery branch is Viber-specific. On setup
  failure, check the tenant's `ViberIntegrations` by the new `inboxId`. If a
  record exists, preserve both records and throw the fixed saved-but-unconfirmed
  message. Only a confirmed absence permits common-inbox rollback. A duplicate
  bot on another inbox must not preserve the new common record. If recovery
  lookup or rollback fails, propagate that error; never speculatively delete
  credentials or another connection. A lost provider-record write acknowledgement
  or webhook response can therefore be retried through Repair without recreation.
...
- `processViberMessage` defaults omitted attachments to an empty array and accepts
  ...
  must match the expected thread and Core customer, with no internal/user origin.
  Repair conversation preview, actual document count, open/unread state, and
  customer-last metadata even when a previous attempt already inserted the row.
  Await `pConversationClientMessageInserted` before setting `processedAt`, and
...
- `downloadViberMedia(source, messageType, allowedHostnames)` validates the URL
  ...
  tenant settings, environment override, or built-in defaults only after
  signature verification. The initial defaults are documented in
  [Gupshup's incoming Viber examples](https://docs.gupshup.io/docs/viber-message-types-inbound);
  additional hosts require review, never automatic approval from a callback.
...
- All saved Viber utility, receiver, mapping schema, and helper tests, from the
  ...
  Receiver tests use the existing plugin aliases and replace the shared tenant
  lookup, model-loader, and Viber processor imports in the CommonJS cache. They restore
  ...
  server, or bot token is required; these receiver tests do not exercise persistence.
...
- Media-policy cases in that same configuration suite cover normalization,
  deduplication, absent environment overrides, tenant forwarding, invalid-entry
  rejection, and rereading changed configuration. They do not verify actual
  environment substitution, provider host ownership, DNS, downloads, or receiver wiring.
- Media-settings tests cover built-in defaults, environment replacement,
  saved overrides and explicit disablement, reset, independent returned arrays,
  and exact-host download matching with mocked fetch. Readiness uses that same
  resolver; these checks do not prove live provider delivery.
...
- Viber creation tests in `__tests__/creation.spec.ts` compose the real resolver,
  adapter, helpers, configuration builder, and HTTP utilities. Mocked persistence
  and fetch verify that both records exist before registration, creation awaits
  completion, pre-save failures roll back only the new common record, and saved
  connections survive ambiguous writes/provider failures for Repair. They also
  cover duplicate-bot isolation, configuration/health-save failures, failed
  recovery lookups or rollback, and unchanged sibling cleanup. These are not
  live MongoDB, GraphQL server, callback delivery, or provider-acceptance tests.
...
- **Affected areas:** Viber constants, settings resolver, configuration/readiness tests and approved-host download coverage.
- **Contracts changed:** `source: default` now returns `dl-media.viber.com` and `content.cdn.viber.com`; GraphQL signatures and storage behavior are unchanged.
