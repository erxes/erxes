...
- Help centers: the client portal config record behind a published help center
  site — its general settings (name, description, website, and the knowledge
  base, ticket and form feature groups plus the CMS behind its announcements),
  its appearance (logo pair, surface colours, fonts,
...
- `core` over tRPC — brands, tags, users, structure,
  ...
  ticket property fields chosen in a ticket config), and `cpUsers.get` (query,
  `{ id }`), which backs `Survey.createdCpUser`.
- `core` over tRPC — messenger companies: `companies.findOne` (query),
  `companies.createCompany` and `companies.updateCompany` (mutations,
  `{ doc }` / `{ _id, doc }`), `customers.createMessengerCustomer` /
  `updateMessengerCustomer`, `conformity.create`, and
  `fields.generatePropertiesData`, which splits messenger `companyData` into
...
- `frontline_help_center_configs` — one document per help center, holding
  ...
  `ticketPipelineId` / `ticketStatusId` group, the `formChannelId` /
  `formIds` pair, the `cmsId` / `cmsAppToken` pair, the `header` wording block
  and the `footer` content block with its link columns) and appearance (`color`,
...
- `models.HelpCenterConfigs.createOrUpdateConfig` writes with `$set` over the whole
  ...
  sends.
- The form pair has no toggle of its own; it rides on `ticketToggle`. A form
  is owned by one channel, and `formChannelId` is independent of
  `ticketChannelId` and optional. `normalizeHelpCenterConfig` deduplicates
  `formIds` and blanks both fields when `ticketToggle` is off;
  `createOrUpdateConfig` then keeps only ids of `Forms` whose
  `channelId` is `formChannelId`, in the order given. A deleted form or one
  from another channel therefore drops out on save instead of blocking it —
  never turn that filter into a thrown error.
- `cmsId` names a `content_api` CMS this plugin never reads; `cmsAppToken` is
  the app token of that CMS's client portal, copied by the UI when the CMS is
  picked, which the published site sends with its `cp*` post queries.
  `normalizeHelpCenterConfig` rejects a `cmsId` without a token and clears the
  token when `cmsId` is empty.
...
### `2026-09-17` — A help center stores the CMS behind its announcements

- **Summary:** Help center configs carry `cmsId` and `cmsAppToken`; a CMS
  without a token is rejected and clearing the CMS clears the token.
- **Affected areas:**
  `src/modules/helpcenter/{@types/helpCenterConfig.ts,db/definitions/helpCenterConfig.ts,graphql/schemas/helpCenterConfig.ts,utils/helpCenterConfig.ts}`
- **Contracts changed:** `HelpCenterConfig` and `HelpCenterConfigInput` gain
  `cmsId: String` and `cmsAppToken: String`.

### `2026-09-17` — A help center stores its forms and their channel

- **Summary:** Help center configs carry an optional `formChannelId` and
  `formIds` filtered to that channel's existing forms on every save, both
  cleared while tickets are off.
- **Affected areas:**
  `src/modules/helpcenter/{@types/helpCenterConfig.ts,db/definitions/helpCenterConfig.ts,db/models/HelpCenterConfig.ts,graphql/schemas/helpCenterConfig.ts,utils/helpCenterConfig.ts}`
- **Contracts changed:** `HelpCenterConfig` and `HelpCenterConfigInput` gain
  `formChannelId: String` and `formIds: [String]`.
