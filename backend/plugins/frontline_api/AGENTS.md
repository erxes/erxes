...
| Call Pro | `src/modules/integrations/callpro/` | `CALLPRO_ENABLED` + `CALLPRO_SUBDOMAINS` gate, `/callpro/receive` webhook, mirrored line/caller/call, recording URL |
...
- HTTP `POST /callpro/receive` — the Call Pro PBX pushes one call event
  ...
  mounted when `CALLPRO_ENABLED=true`, so a deployment without Call Pro returns 404;
  a mounted route also answers 404 to a subdomain outside `CALLPRO_SUBDOMAINS`. Public URL: `{DOMAIN}/gateway/pl:frontline/callpro/receive`
...
- Call Pro stays invisible unless `CALLPRO_ENABLED=true`. On a shared (SaaS)
  deployment `CALLPRO_SUBDOMAINS` (comma-separated, case-insensitive) narrows it
  to those tenants; left empty, every tenant gets it. `isCallProEnabled(subdomain)`
  is the only per-tenant check and gates the webhook handler, the create/update
  handlers, `callProAudio`, and — through `callProConfig` — every UI surface;
  `isCallProInstalled()` only decides whether the route is mounted. It is independent of the
...
- Smoke: `GET /agent-tools/manifest` on the frontline service lists only the
  ...
  appear.
- Smoke: with `CALLPRO_ENABLED` unset, `POST /callpro/receive` must 404, and with
  `CALLPRO_SUBDOMAINS=emartnew` any other subdomain must 404 and see no Call Pro
  card. With it set to `true`, create a Call Pro line and post
...
- **Summary:** `CALLPRO_SUBDOMAINS` restricts Call Pro to the listed subdomains on
  a shared deployment; other tenants get `callProConfig.enabled = false`, a 404
  webhook, and rejected create/update calls.
- **Affected areas:** `src/modules/integrations/callpro/{config,routes,messageBroker}.ts`,
  `src/modules/integrations/callpro/graphql/resolvers/queries.ts`,
  `src/modules/inbox/graphql/resolvers/customResolvers/conversation.ts`,
  `src/routes.ts`
- **Contracts changed:** New optional env `CALLPRO_SUBDOMAINS`; `POST /callpro/receive`
  returns 404 for subdomains outside it.
