...
export const ticketExportHandlers: TExportHandlers = {
  getExportHeaders: async (data: any, ctx: IImportExportContext) => {
    ...
    const handler = ticketExportMap[collectionName as keyof typeof ticketExportMap]?.getExportHeaders;
  ...
  getExportData: async (args: any, ctx: IImportExportContext) => {
    ...
    const handler = ticketExportMap[collectionName as keyof typeof ticketExportMap]?.getExportData;
