...
const loadViberRecipient = async (
  context: IContext,
  conversationId: string,
  access?: Awaited<ReturnType<typeof assertViberConversationAccess>>,
) => {
...
export const sendViberReply = async (
  ...
  try {
    ...
    const access = await assertViberConversationAccess(
      context,
    ...
    // Only this attempt is known not to have written. A client recovering an
    // earlier lost response must retain that earlier request even on this code.
    throw new GraphQLError(
      error instanceof Error ? error.message : 'Unable to prepare Viber reply',
      {
        extensions: { code: 'VIBER_SEND_NOT_SAVED' },
      },
    );
  }
};
