...
export const removeIntegration = async (
  ...
  if (kind.includes("instagram")) {
    ...
    try {
      ...
      await models.InstagramPostConversations.deleteMany({ recipientId: pageId });
      await models.InstagramCommentConversation.deleteMany({ recipientId: pageId });
    ...
    const conversationIds =
      await models.InstagramConversation.find(selector).distinct("_id");

    await models.InstagramCustomers.deleteMany({ integrationId: integrationErxesApiId });
    await models.InstagramConversation.deleteMany(selector);
    await models.InstagramConversationMessage.deleteMany({
...
export const removeCustomers = async (models: IModels, params) => {
  ...
  await models.InstagramCustomers.deleteMany(selector);
};
...
export const updateConfigs = async (
  ...
  await models.InstagramConfig.updateConfigs(configsMap);
...
export const instagramGetCustomerPosts = async (
  ...
  const customer = await models.InstagramCustomers.findOne({ erxesApiId: customerId });
