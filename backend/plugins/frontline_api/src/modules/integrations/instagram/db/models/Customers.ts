...
type InstagramCustomerDoc =
  HydratedDocument<IInstagramCustomerDocument>;

type InstagramCustomerLean =
...
export interface IInstagramCustomerModel
  ...
  getCustomer(
    selector: any,
    isLean: true
  ): Promise<InstagramCustomerLean>;

  getCustomer(
    selector: any,
    isLean?: false
  ): Promise<InstagramCustomerDoc>;
}
...
export const loadInstagramCustomerClass = (models: IModels) => {
  class Customer {
    public static async getCustomer(
      ...
      if (isLean) {
        const customer =
          await models.InstagramCustomers
            .findOne(selector)
      ...
      const customer =
        await models.InstagramCustomers.findOne(selector);
