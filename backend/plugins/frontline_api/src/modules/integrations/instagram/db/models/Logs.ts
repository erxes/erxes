...
export interface IInstagramLogModel extends Model<IInstagramLogDocument> {
  createLog({ type, value, specialValue }: IInstagramLogInput): IInstagramLogDocument;
}

export const loadInstagramLogClass = (models: IModels) => {
  class Log {
    public static async createLog({ type, value, specialValue }: IInstagramLogInput) {
