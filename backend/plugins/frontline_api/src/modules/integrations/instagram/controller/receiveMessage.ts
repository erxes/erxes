...
// import { debugInfo } from "erxes-api-shared/src/utils";
...
const receiveMessage = async (
  ...
  let conversationMessage = await models.InstagramConversationMessage.findOne({
    mid: mid
  });
  if (!conversationMessage) {
    if (adData) {
      const adsMessage = await models.InstagramConversationMessage.addMessage({
    ...
    // save on integrations db
    try {
      const createdMessage = await models.InstagramConversationMessage.create({
        mid: mid,
        timestamp,
        senderId: userId,
        recipientId: recipient.id,
    ...
    const updatedMessage = await models.InstagramConversationMessage.findOneAndUpdate(
      { mid: mid },
