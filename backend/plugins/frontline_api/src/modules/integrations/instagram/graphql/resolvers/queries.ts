...
import { IPersistentMenus } from '@/integrations/instagram/@types/bots'
...
export const instagramQueries = {
  ...
  async instagramGetComments(
    ...
    if (senderId && senderId !== "undefined") {
      const customer = await models.InstagramCustomers.findOne({ erxesApiId: senderId });
  ...
  async instagramGetCommentCount(_root, args, { models }: IContext) {
    ...
    const commentCount = await models.InstagramCommentConversation.countDocuments({
      erxesApiId: conversationId
    });
  ...
  async instagramConversationDetail(
    ...
    const conversation = await models.InstagramConversation.findOne({ _id });
    if (conversation) {
      return conversation;
    }
  ...
  async instagramConversationMessages(
    ...
    const query = await buildSelector(
        conversationId, 
        models.InstagramConversation);
    if (conversation) {
      if (limit) {
        const sort: any = getFirst ? { createdAt: 1 } : { createdAt: -1 };

        messages = await models.InstagramConversationMessage.find(query)
          .sort(sort)
          .skip(skip || 0)
          .limit(limit);
  ...
  async instagramConversationMessagesCount(
    ...
    return models.InstagramConversationMessage.countDocuments(selector);
  },
  ...
  async instagramMessengerBots(_root, _args, { models }: IContext) {
    try {
      const bots = await models.InstagramBots.find({});
      const result = await Promise.all(
        bots.map(async (bot) => {
          ...
          const ig_account = await models.InstagramAccounts.getAccount({
            _id: bot.accountId
          }).catch(() => null);
  ...
  async instagramHasTaggedMessages(
    ...
    const inboxConversation = await models.InstagramConversation.findOne({
        _id: conversationId,
    })
  ...
  async instagramPostMessages(
    ...
    const { conversationId, limit, skip, getFirst } = args;
    let messages: any[] = [];
    const query = await buildSelector(conversationId, models.InstagramPostConversations);
