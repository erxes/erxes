...
} from '@/integrations/instagram/@types/conversations';
import {
  IInstagramCustomer,
  IInstagramCustomerDocument,
} from '@/integrations/instagram/@types/customers';
...
export const generateMessages = async ({
  ...
  const generateButtons = (buttons: TBotConfigMessageButton[] = []) => {
    const generatedButtons: TInstagramMessageButton[] = [];

    for (const button of buttons) {
      const obj: TInstagramMessageButton = {
        type: 'postback',
...
export const generateObjectToWait = ({
  ...
  const propertyName = 'payload.btnId';
...
export const getData = async (
  ...
  const bot = await models.InstagramBots.findOne({ _id: botId }, { tag: 1 });
