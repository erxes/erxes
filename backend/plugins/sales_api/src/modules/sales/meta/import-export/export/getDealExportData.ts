...
export async function getDealExportData(
  ...
  if (filters && Object.keys(filters).length > 0) {
    ...
    if (f.assignedUserId) query.assignedUserIds = { $in: [f.assignedUserId] };
  ...
  const stages = stageIds.size
    ? await models.Stages.find({ _id: { $in: [...stageIds] } })
  ...
  for (const s of stages as any[]) {
    if (s.pipelineId) pipelineIds.add(s.pipelineId);
  }

  const pipelines = pipelineIds.size
    ? await models.Pipelines.find({ _id: { $in: [...pipelineIds] } })
        .select('_id name')
        .lean()
    : [];

  const labels = labelIds.size
    ? await models.PipelineLabels.find({ _id: { $in: [...labelIds] } })
        .select('_id name')
        .lean()
    : [];
