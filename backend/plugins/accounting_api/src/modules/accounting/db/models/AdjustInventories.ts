...
export const loadAdjustInventoriesClass = (
  ...
  class AdjustInventories {
    ...
    public static async updateAdjustInventory(
      _id: string,
      doc: IAdjustInventory,
    ) {
    ...
    public static async removeAdjustInventory(_id: string) {
      ...
      if (
        ![ADJ_INV_STATUSES.DRAFT, ADJ_INV_STATUSES.PROCESS].includes(
          adjusting.status,
        )
      ) {
        throw new Error(
          'this adjusting cannot be delete yet, it has not been draft or cancel.',
        );
      }
...
export interface IAdjustInvDetailsModel extends Model<IAdjustInvDetailDocument> {
  ...
  getAdjustInvDetailInfo(
    args: IAdjustInvDetailParamsId,
  ): Promise<IAdjustInvDetail>;
  copyAdjustInvDetails(args: {
    sourceAdjustId: string;
    adjustId: string;
  }): Promise<void>;
  ...
  increaseAdjustInvDetail(
    args: IAdjustInvDetailParamsId & {
      count: number;
      amount: number;
      adjustDetail?: IAdjustInvDetailDocument;
      multiplier?: number;
      hasResp?: boolean;
    },
  ): Promise<IAdjustInvDetail | void>;
...
export const loadAdjustInvDetailsClass = (
  ...
  class AdjustInvDetails {
    public static async getAdjustInvDetail({
      productId,
      accountId,
      departmentId,
      branchId,
    ...
    public static async copyAdjustInvDetails({
      sourceAdjustId,
      adjustId,
    }: {
      sourceAdjustId: string;
      ...
      const preAdjustDetailsCount = await models.AdjustInvDetails.find({
      ...
      while (step * per <= preAdjustDetailsCount) {
        ...
        const sourceDetails = await models.AdjustInvDetails.find({
          adjustId: sourceAdjustId,
          $or: [{ remainder: { $eq: 0 } }, { cost: { $eq: 0 } }],
        })
          .skip(skip)
          .limit(per)
          .lean();

        await models.AdjustInvDetails.insertMany(
    ...
    public static async getAdjustInvDetailInfo({
      productId,
      accountId,
      departmentId,
      branchId,
      ...
      const lastPublishAdjust = await models.AdjustInventories.findOne({
