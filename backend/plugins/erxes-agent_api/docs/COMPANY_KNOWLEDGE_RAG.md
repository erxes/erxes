...
Three rules:
1. **Store *resource* permissions in the payload, not *user* permissions.** Each vector carries
...
2. **Resolve the *user's* current permissions at query time**, then filter. Makes revocation instant.
...
- Batch embed calls + debounce; only embed *meaningful* field changes, not every tweak.
...
**erxes reality:** permissions are *action-level* (module access) **+** *scope-level* (branch /
...
enough and far cheaper. Permission *revocation* is instant regardless (resolved live at query time).
**Notes:** _(to fill in)_
...
3. Wire **pre-filter + erxes-permission post-filter** into a `companyKnowledge` retrieval tool the
   agent can call.
4. **Prove leakage is impossible** with a low-privilege test user *before* scaling to more content
   types.
