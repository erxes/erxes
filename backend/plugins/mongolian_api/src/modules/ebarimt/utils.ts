...
import { fixNum } from '../../../../../erxes-api-shared/src/utils';
import { sendTRPCMessage } from "../../../../../erxes-api-shared/src/utils";
import { IModels } from "./connectionResolver";


const callCore = async (actionStr: string, data?: any, defaultValue?: any) => {
  let moduleName = "core";
  let action = actionStr;

  if (actionStr.includes(".")) {
    const [m, a] = actionStr.split(".");
    moduleName = m;
    action = a;
  }
...
export const getConfig = async (code: string, defaultValue?: any) => {
  return await callCore("getConfig", { code, defaultValue }, defaultValue);
};

/**
 * Validate company code via external checker (EBARIMT service URL in config)
 */
export const validCompanyCode = async (config: any, companyCode: string) => {
...
/**
 * Check and possibly enrich params.primaryName using company code.
 */
export const companyCheckCode = async (params: any) => {
  if (!params.code) {
    return params;
  }
  ...
  if (!config || !config.checkTaxpayerUrl || !config.checkTaxpayerUrl.includes("http")) {
    return params;
  }
...
export const validConfigMsg = async (config: any) => {
  if (!config.url) {
    return "required url";
  }
  return "";
...
const billTypeCustomFieldsData = async (config: any, deal: any) => {
  if (
...
const billTypeConformityCompany = async (config: any, deal: any) => {
  const companyIds = await callCore("conformities.savedConformity", {
    mainType: "deal",
    mainTypeId: deal._id,
    relTypes: ["company"],
  }, []);
  ...
  if (companyIds.length > 0) {
    const companies = await callCore("companies.findActiveCompanies", {
...
const checkBillType = async (config: any, deal: any) => {
  ...
  if (type === "B2C_RECEIPT") {
    const checkerC = await billTypeConformityCompany(config, deal);
  ...
  if (type === "B2C_RECEIPT") {
    const customerIds = await callCore("conformities.savedConformity", {
      mainType: "deal",
      mainTypeId: deal._id,
      relTypes: ["customer"],
    }, []);
    ...
    if (customerIds.length > 0) {
      const customers = await callCore("customers.findActiveCustomers", {
      ...
      let customer = customers.find((c: any) => c.code && c.code.match(/^\d{8}$/g));
...
const getChildCategories = async (categoryIds: string[]) => {
  const childs = await callCore("categories.withChilds", { ids: categoryIds }, []);
  const catIds: string[] = (childs || []).map((ch: any) => ch._id) || [];
  return Array.from(new Set(catIds));
};

const getChildTags = async (tagIds: string[]) => {
  const childs = await callCore("tagWithChilds", {
    query: { _id: { $in: tagIds } },
    fields: { _id: 1 },
  }, []);

  const foundTagIds: string[] = (childs || []).map((ch: any) => ch._id) || [];
  return Array.from(new Set(foundTagIds));
};

const checkProductsByRule = async (products: any[], rule: any) => {
  let filterIds: string[] = [];
  const productIds = products.map((p) => p._id);

  if (rule.productCategoryIds?.length) {
    const includeCatIds = await getChildCategories(rule.productCategoryIds);
  ...
  if (rule.tagIds?.length) {
    const includeTagIds = await getChildTags(rule.tagIds);
  ...
  if (rule.excludeCatIds?.length) {
    const excludeCatIds = await getChildCategories(rule.excludeCatIds);
  ...
  if (rule.excludeTagIds?.length) {
    const excludeTagIds = await getChildTags(rule.excludeTagIds);
...
const calcProductsTaxRule = async (models: IModels, config: any, products: any[]) => {
  try {
    ...
    if (vatRules.length) {
      for (const rule of vatRules) {
        const productIdsByRule = await checkProductsByRule(products, rule);
...
const calcPreTaxPercentage = (paymentTypes: any[], deal: any) => {
...
/**
 * getPostData (no subdomain)
 */
export const getPostData = async (models: IModels, config: any, deal: any, paymentTypes: any[]) => {
  ...
  const firstProducts = await callCore("products.find", {
    query: { _id: { $in: productsIds } },
    limit: productsIds.length,
  }, []);

  const productsById = await calcProductsTaxRule(models, config, firstProducts);
...
export const getCompanyInfo = async ({ checkTaxpayerUrl, no }: { checkTaxpayerUrl: string; no: string; }) => {
  ...
  if (tinre.test(no)) {
    const result = await fetch(`${checkTaxpayerUrl}/getInfo?tin=${no}`).then((r) => r.json());
  ...
  const info = await fetch(`${checkTaxpayerUrl}/getTinInfo?regNo=${no}`).then((r) => r.json());
  ...
  const result = await fetch(`${checkTaxpayerUrl}/getInfo?tin=${tinNo}`).then((r) => r.json());

  return { status: "checked", result, tin: tinNo };
};

export const returnResponse = async (url: string, data: any) => {
