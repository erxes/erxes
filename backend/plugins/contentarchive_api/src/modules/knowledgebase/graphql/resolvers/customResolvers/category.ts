...
import { ICategoryDocument } from '@/knowledgebase/@types/knowledgebase';
import { PUBLISH_STATUSES } from '@/knowledgebase/constants';
...
export const KnowledgeBaseCategory = {
  ...
  async numOfArticles(category: ICategoryDocument, args, { models }: IContext) {
    const { status } = args;
    const query = {
      categoryId: category._id,
      status: {
