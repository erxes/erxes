...
import { IAssignmentCampaign } from '~/modules/loyalty/@types/assignmentCampaigns';
import {
  putCreateLog,
  putDeleteLog,
  putUpdateLog,
...
const assignmentsMutations = {
  ...
  async assignmentCampaignsRemove(
    ...
    const assignmentCampaign: IAssignmentCampaign[] =
      await models.AssignmentCampaigns.find({
        _id: { $in: _ids },
      }).lean();
