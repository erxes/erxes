...
//
// Two backends, selected by MASTRA_EMBEDDER:
...
async function buildFastEmbedEmbedder(cfg: EmbedderConfig): Promise<Embedder> {
  ...
  try {
    ...
    throw new Error(
      '[mastra:memory] The local embedder needs the "fastembed" package. ' +
        'Install it in backend/plugins/mastra_api, or set MASTRA_EMBEDDER=openai.',
    );
  }
  ...
  const cacheDir = (process.env.MASTRA_EMBEDDER_CACHE_DIR ?? '').trim() || undefined;
