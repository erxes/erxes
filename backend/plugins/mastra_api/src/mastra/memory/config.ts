...
/**
 * The master switch. Advanced memory is enabled ONLY when MASTRA_MEMORY is
...
export function isAdvancedMemoryEnabled(env: Env = process.env): boolean {
  return val(env, 'MASTRA_MEMORY') === 'enable';
}
...
export function resolveEmbedderConfig(env: Env = process.env): EmbedderConfig {
  const raw = val(env, 'MASTRA_EMBEDDER').toLowerCase();
  ...
  if (raw === '' || raw === 'fastembed') {
    ...
    console.warn(
      `[mastra:memory] Unknown MASTRA_EMBEDDER="${raw}", defaulting to fastembed.`,
    );
    kind = 'fastembed';
  }

  if (kind === 'fastembed') {
    const model = val(env, 'MASTRA_EMBEDDER_MODEL') || FASTEMBED_DEFAULT_MODEL;
    const dimension = EMBED_DIMENSIONS[model] ?? FASTEMBED_FALLBACK_DIM;
    return { kind, model, dimension };
  }

  const model = val(env, 'MASTRA_EMBEDDER_MODEL') || OPENAI_DEFAULT_MODEL;
  const dimension = EMBED_DIMENSIONS[model] ?? OPENAI_FALLBACK_DIM;
  const baseUrl = val(env, 'MASTRA_EMBEDDER_BASE_URL') || OPENAI_DEFAULT_BASE_URL;
  const apiKey = val(env, 'MASTRA_EMBEDDER_API_KEY') || undefined;
...
export function resolveRecallTuning(env: Env = process.env): RecallTuning {
  return {
    topK: parsePositiveInt(val(env, 'MASTRA_MEMORY_TOPK'), 4),
    minScore: parseScore(val(env, 'MASTRA_MEMORY_MIN_SCORE'), 0.5),
    scope: val(env, 'MASTRA_MEMORY_SCOPE').toLowerCase() === 'thread'
      ? 'thread'
      : 'resource',
  };
}
...
export function qdrantUrl(env: Env = process.env): string {
  return val(env, 'MASTRA_QDRANT_URL') || DEFAULT_QDRANT_URL;
}

export function qdrantApiKey(env: Env = process.env): string | undefined {
  return val(env, 'MASTRA_QDRANT_API_KEY') || undefined;
}
