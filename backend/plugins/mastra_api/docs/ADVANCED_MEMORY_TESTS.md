...
- `backend/plugins/mastra_api/jest.config.ts` (preset `../../../jest.preset.js`, `testEnvironment: 'node'`, ts-jest).
- `backend/plugins/mastra_api/tsconfig.spec.json`.
- a `test` target in `project.json` so `pnpm nx test mastra_api` works.
- Unit tests live in `src/**/__tests__/*.test.ts`. Integration tests are tagged
  `*.int.test.ts` and skipped unless `MASTRA_QDRANT_URL` is reachable (guarded `describe`).

**T-HARNESS-1 (U, P0):** `pnpm nx test mastra_api` runs and reports a green suite with at least one
...
| AM-FLAG-2 | U·P0 | `isAdvancedMemoryEnabled({ MASTRA_MEMORY: 'enable' })` | `true` |
| AM-FLAG-3 | U·P0 | `isAdvancedMemoryEnabled({ MASTRA_MEMORY: 'true' })`, `'1'`, `'on'`, `'ENABLE'` | `false` for all (only exact `'enable'` enables) |
| AM-FLAG-4 | U·P1 | `isAdvancedMemoryEnabled({ MASTRA_MEMORY: ' enable ' })` (whitespace) | `true` (trimmed) — document the trim decision |
| AM-GATE-1 | I·P0 | Boot plugin with `MASTRA_MEMORY` unset; spy on the memory façade module loader | Advanced-memory modules are **never imported**; no Qdrant connection attempted; no embed model downloaded |
...
| AM-EMB-2 | U·P0 | `resolveEmbedderConfig({ MASTRA_EMBEDDER:'openai' })` | `{ kind:'openai', model:'text-embedding-3-small', dimension:1536, baseUrl:'https://api.openai.com/v1' }` |
| AM-EMB-3 | U·P1 | `resolveEmbedderConfig({ MASTRA_EMBEDDER:'openai', MASTRA_EMBEDDER_MODEL:'text-embedding-3-large' })` | dimension resolves to `3072` (model→dim table) |
| AM-EMB-4 | U·P1 | Unknown `MASTRA_EMBEDDER` value | falls back to `fastembed` default + a warning |
...
| AM-REC-5 | U·P1 | `resolveRecallTuning({ MASTRA_MEMORY_TOPK:'7', MASTRA_MEMORY_MIN_SCORE:'0.3' })` | `{ topK:7, minScore:0.3, scope:'resource' }`; garbage values → defaults |
...
| AM-REC-8 | E2E·P1 | Same as AM-REC-7 but `MASTRA_MEMORY_SCOPE=thread` | t2 does **not** recall t1's deadline (thread isolation) |
...
| AM-SET-2 | U·P0 | `computeAdvancedMemoryStatus({MASTRA_MEMORY:'enable'}, {reachable:true})` | `{ enabled:true, embedder:'fastembed', qdrantReachable:true, collection:'mastra_memory_…' }` |
...
| ID | Type/Pri | Case | Expected |
|---|---|---|---|
| AM-DOCK-1 | M·P0 | `docker compose -f backend/plugins/mastra_api/docker-compose.yml config` | valid; service `qdrant`, pinned image tag, ports 6333/6334, named volume |
...
| AM-DEP-2 | U·P0 | `pnpm nx build mastra_api` (typecheck) | clean, exit 0 |
...
1. ✅ `pnpm nx test mastra_api` is green and `pnpm nx build mastra_api` typechecks clean.
...
5. ✅ With `MASTRA_MEMORY` unset, behavior is byte-identical to pre-feature (regression suite green).

### Per-phase exit criteria
- **Phase 0 done:** T-HARNESS-1, AM-FLAG-1..4, AM-SET-1/2/4, AM-DEP-2.
- **Phase 1 done:** AM-EMB-*, AM-COL-*, AM-VEC-*, AM-DOCK-*, AM-GATE-1, AM-DEG-1/5, AM-SET-5/6.
- **Phase 2 done:** AM-TEN-*, AM-REC-*, AM-CONV-*, AM-DEG-2/3/4.
- **Phase 3 done:** AM-WM-*.
