...
> Status: **SPEC (pre-development)** · Plugin: `mastra_api` (port 3312) · Owner: erxes Mastra plugin
>
...
2. **Single flag, env-controlled.** `MASTRA_MEMORY=enable` turns it on. Anything else / unset = off.
...
Both turn on together behind the one `MASTRA_MEMORY=enable` flag.
...
## 6. Configuration (all `MASTRA_*` env keys)

| Env var | Default | Purpose |
|---|---|---|
| `MASTRA_MEMORY` | _(unset)_ | `enable` turns advanced memory on. Anything else = off. |
| `MASTRA_QDRANT_URL` | `http://localhost:6333` | Qdrant REST endpoint. |
| `MASTRA_QDRANT_API_KEY` | _(unset)_ | Optional Qdrant API key. |
| `MASTRA_EMBEDDER` | `fastembed` | `fastembed` (local) or `openai` (API). |
| `MASTRA_EMBEDDER_MODEL` | `bge-small-en-v1.5` (fastembed) / `text-embedding-3-small` (openai) | Embedding model id. |
| `MASTRA_EMBEDDER_BASE_URL` | `https://api.openai.com/v1` | OpenAI-compatible base URL (openai mode). |
| `MASTRA_EMBEDDER_API_KEY` | _(unset)_ | Key for the API embedder (openai mode). |
| `MASTRA_MEMORY_TOPK` | `4` | Semantic recall: snippets retrieved per turn. |
| `MASTRA_MEMORY_MIN_SCORE` | `0.5` | Minimum cosine score to include a recalled snippet. |
| `MASTRA_MEMORY_SCOPE` | `resource` | `resource` or `thread` for semantic recall. |
...
`.env.sample` (repo root) gets the keys above, commented, with `MASTRA_MEMORY` disabled by default.

---

## 7. Qdrant Docker — in plugin scope

`backend/plugins/mastra_api/docker-compose.yml`:

```yaml
# Qdrant for the Mastra "Advanced memory feature". Only needed when
# MASTRA_MEMORY=enable. Run:  docker compose -f backend/plugins/mastra_api/docker-compose.yml up -d
services:
  qdrant:
...
- New **computed, read-only** GraphQL fields on `MastraSettings` (type only, **not** the input):
  - `advancedMemory: Boolean!` — `process.env.MASTRA_MEMORY === 'enable'`.
...
- `GeneralSettingsPage.tsx` renders a **disabled** control (lock icon + "Advanced memory feature"
  ...
  `MASTRA_MEMORY` environment variable." No editable input — matches the user's "untouchably" ask.

---

## 9. Boot behavior (`main.ts` → `onServerInit`)

When `MASTRA_MEMORY=enable`:
...
**New files (all in `backend/plugins/mastra_api/`):**
...
- `frontend/plugins/mastra_ui/src/graphql/queries.ts` — `MASTRA_SETTINGS` gets the new read-only fields.
- `frontend/plugins/mastra_ui/src/pages/settings/GeneralSettingsPage.tsx` — read-only status block.
- repo `.env.sample` — documented `MASTRA_*` keys (disabled by default).
...
2. **FastEmbed (the default local embedder) is an optional, lazily-loaded dependency.** It is the
   ...
   and the **OpenAI embedder works dep-free** (`MASTRA_EMBEDDER=openai`). Adding `fastembed` is a
...
docker compose -f backend/plugins/mastra_api/docker-compose.yml up -d

# Enable in .env
MASTRA_MEMORY=enable
MASTRA_QDRANT_URL=http://localhost:6333
MASTRA_EMBEDDER=fastembed        # or: openai (+ MASTRA_EMBEDDER_API_KEY)
