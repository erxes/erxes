...
function matches(target: Target, filters: string[]): boolean {
  ...
  return filters.some((f) => {
    ...
    return (
      ...
      target.rel.replace(/\.ts$/, '') === norm || // domain/commands/file
      `${target.domain}/${target.file.replace(/\.ts$/, '')}` === norm || // domain/file
...
function main(): void {
  ...
  if (filters.length && targets.length === 0) {
    console.error(`No commands matched: ${filters.join(', ')}`);
    console.error(`Available:\n  ${all.map((t) => t.rel).join('\n  ')}`);
    process.exit(1);
  }

  console.log(`Found ${targets.length} command(s) to run:`);
