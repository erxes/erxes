...
import { IConfig, IConfigDocument, ISESConfig } from '../../@types/types';
import { Schema } from 'mongoose';

export const configsSchema = new Schema({
  code: { type: String, label: 'Code', unique: true, required: true },
  value: { type: String, label: 'Value', required: true },
});

export interface IConfigModel extends Model<IConfigDocument> {
  getConfig(code: string): Promise<IConfigDocument>;
  updateConfigs(configsMap: Record<string, string[]>): Promise<void>;
