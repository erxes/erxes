...
export interface ITemplateCategoryModal extends Model<ITemplateCategoryDocument> {
    ...
    createTemplateCategory(category: ITemplateCategory, user: IUserDocument): Promise<ITemplateCategoryDocument>
    updateTemplateCategory(_id: string, category: ITemplateCategory, user: IUserDocument): Promise<ITemplateCategoryDocument>
...
export const loadTemplateCategoryClass = (models: IModels) => {
    class Category {
        ...
        public static async removeTemplateCategory(
            _id: string
        ) {
            const category = await models.TemplateCategory.getTemplateCategory(_id)
            
            return await models.TemplateCategory.findOneAndDelete({ _id: category._id }) 
        }

        public static async validateTemplateCategory({ _id, category }: { _id?: string, category: ITemplateCategory}) {
            ...
            if (parentId && _id) {
                const category = await models.TemplateCategory.findOne({ parentId }).lean();
