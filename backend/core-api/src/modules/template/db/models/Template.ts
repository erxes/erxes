...
export interface ITemplateModal extends Model<ITemplateDocument> {
  ...
  createTemplate(template: ITemplate, user: IUserDocument): Promise<ITemplateDocument>;
  updateTemplate(_id: string, template: ITemplate, user?: IUserDocument): Promise<ITemplateDocument>;
...
export const loadTemplateClass = (models: IModels, subdomain: string) => {
    class Template {
        ...
        public static async createTemplate(template: ITemplate, user: IUserDocument) {
            const document = {
                ...template,
                createdBy: user._id
            }

            const relatedContents = await getRelatedContents(document, subdomain) || []
        ...
        public static async updateTemplate(_id: string, template: ITemplate, user: IUserDocument) {
