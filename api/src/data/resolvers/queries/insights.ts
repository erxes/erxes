...
import {
  getDateFieldAsStr,
  getDurationField
} from '../../modules/insights/aggregationUtils';
...
const insightQueries = {
  ...
  async insightsTags(_root, args: IListArgs, { user }: IContext) {
    ...
    const tags = await Tags.find({ type: TAG_TYPES.CONVERSATION }).select(
      'name'
    );

    const integrationIdsByTag = await Integrations.findIntegrations(
      filterSelector.integration
    ).select('_id');
  ...
  async insightsFirstResponse(_root, args: IListArgs, { user }: IContext) {
    ...
    const selector = await getConversationSelector(
      filterSelector,
      conversationSelector
    );
    ...
    for (const conversation of conversations) {
      ...
      if (firstRespondedDate && firstRespondedUserId) {
        ...
        if (responseUserData[userId]) {
          responseUserData[userId].responseTime =
            responseTime + responseUserData[userId].responseTime;
        ...
        responseUserData[userId].summaries[index] =
          responseUserData[userId].summaries[index] + 1;
      }
    }

    const doc = await generateResponseData(
      firstResponseData,
      responseUserData,
      allResponseTime
    );
   ...
   */
  async insightsConversationSummary(
    _root,
    args: IListArgs,
    { user }: IContext
  ) {
    ...
    const conversationSelector = await getConversationSelectorByMsg(
      integrationIds,
      brandIds
    );
    ...
    const summaryChart = await fixChartData(
      insightAggregateData,
      'date',
      'avgSecond'
    );

    const userIds = await insightAggregateData
    ...
    for (const userId of userIds) {
      const perData = insightAggregateData.filter(
   ...
   */
  async insightsConversationCustomerAvg(
    _root,
    args: IListArgs,
    { user }: IContext
  ) {
    ...
    return [
      {
        ...
        count: insightAggregateCustomer.length
          ? insightAggregateCustomer[0].avgSecond
          : 0
      }
    ];
  },
   ...
   */
  async insightsConversationInternalAvg(
    _root,
    args: IListArgs,
    { user }: IContext
  ) {
    ...
    const lookupHelper = await getConversationReportLookup();
    lookupHelper.lookupPrevMsg.$lookup.pipeline[1].$project.sizeMentionedIds = {
      $size: '$mentionedUserIds'
    };
    ...
    return [
      {
        ...
        count: insightAggregateInternal.length
          ? insightAggregateInternal[0].avgSecond
          : 0
      }
    ];
  },
   ...
   */
  async insightsConversationOverallAvg(
    _root,
    args: IListArgs,
    { user }: IContext
  ) {
    ...
    const conversationMatch = await getConversationSelectorToMsg(
      integrationIds,
      brandIds
    );
    ...
    const insightAggregateDurationAvg = await Conversations.aggregate([
      ...
      { $unwind: '$lastMsg' },
      {
        $addFields: {
          diffSec: {
            $divide: [
              { $subtract: ['$lastMsg.createdAt', '$firstMsg.createdAt'] },
              1000
            ]
          }
        }
    ...
    return [
      {
        ...
        count: insightAggregateAllAvg.length
          ? insightAggregateAllAvg[0].avgSecond
          : 0
      },
      {
        title: 'Duration time up until the conversation is resolved',
        count: insightAggregateDurationAvg.length
          ? insightAggregateDurationAvg[0].avgSecond
          : 0
      }
    ];
  }
