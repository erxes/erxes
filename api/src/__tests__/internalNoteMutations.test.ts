...
describe('InternalNotes mutations', () => {
  ...
  test('Add internal note', async () => {
    ...
    let internalNote = await graphqlRequest(
      addMutation,
      'internalNotesAdd',
      args,
      context
    ...
    internalNote = await graphqlRequest(
      addMutation,
      'internalNotesAdd',
      args,
      context
  ...
  test('Edit internal note', async () => {
    ...
    const note = await internalNoteFactory({
      contentType: MODULE_NAMES.CUSTOMER,
      contentTypeId: customer._id
    });
    const dealNote = await internalNoteFactory({
      contentType: MODULE_NAMES.DEAL,
      contentTypeId: deal._id
    });
  ...
  test('Remove internal note', async () => {
    ...
    const note1 = await internalNoteFactory({
      contentType: MODULE_NAMES.DEAL,
      contentTypeId: deal._id
    });
    const note2 = await internalNoteFactory({
      contentType: MODULE_NAMES.COMPANY,
      contentTypeId: company._id
    });
    const note3 = await internalNoteFactory({
      contentType: MODULE_NAMES.TASK,
      contentTypeId: task._id
    });
    const note4 = await internalNoteFactory({
      contentType: MODULE_NAMES.TICKET,
      contentTypeId: ticket._id
    });
    const note5 = await internalNoteFactory({
      contentType: MODULE_NAMES.GROWTH_HACK,
      contentTypeId: hack._id
    });
    const note6 = await internalNoteFactory({
      contentType: MODULE_NAMES.USER,
      contentTypeId: _user._id
    });
    const note7 = await internalNoteFactory({
      contentType: MODULE_NAMES.PRODUCT,
