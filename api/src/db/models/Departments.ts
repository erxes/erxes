...
import { IDepartmentDocument, departmentSchema } from './definitions/departments';
...
export interface IDepartmentModel extends Model<IDepartmentDocument> {
    ...
    updateDepartment(_id: string, doc: any, user: IUserDocument): IDepartmentDocument;
  ...
  export const loadClass = () => {
    class Department {
      ...
      */
      public static async updateDepartment(_id: string, doc: any, user: IUserDocument) {
        await Departments.update({ _id }, {
          ...doc,
          updatedAt: new Date(),
          updatedBy: user._id
        });
...
const Departments = model<IDepartmentDocument, IDepartmentModel>('departments', departmentSchema);
