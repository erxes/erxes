...
import { CMS_TAGS, CMS_TAGS_EDIT, CMS_TAGS_REMOVE } from '../graphql/queries';
  ...
  const columns: ColumnDef<any>[] = [
    checkboxColumn,
    {
      ...
      cell: ({ cell }) => {
        ...
        const onSave = async () => {
          if (editingCell && editingCell.value !== original.name) {
            await editTag({
              variables: {
                _id: original._id,
                input: { name: editingCell.value },
              },
            });
          }
          setEditingCell(null);
            ...
            <Popover
              ...
              onOpenChange={(v) => {
                if (!v) {
                  ...
                  setEditingCell({ rowId, value: currentValue });
                }
              }}
            >
              ...
              <RecordTableInlineCell.Content>
                <Input
                  value={currentValue}
                  onChange={(e) =>
                    setEditingCell({ rowId, value: e.currentTarget.value })
                  }
                />
              </RecordTableInlineCell.Content>
            </Popover>
