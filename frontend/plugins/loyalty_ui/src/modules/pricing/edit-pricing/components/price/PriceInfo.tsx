...
export const PriceInfo = ({ pricingId, pricingDetail }: PriceInfoProps) => {
  const [enabled, setEnabled] = useState<boolean>(false);
  const [rules, setRules] = useState<PriceRuleConfig[]>([]);
  const [editingRule, setEditingRule] = useState<PriceRuleConfig | null>(null);
  ...
  useEffect(() => {
    if (pricingDetail) {
      setEnabled(pricingDetail.isPriceEnabled ?? false);
      const existingRules = pricingDetail.priceRules?.map((rule, index) => ({
        _id: `rule_${index}`,
  ...
  const handleRuleAdded = (rule: PriceRuleConfig) => {
    setRules((prev) => [
      ...prev,
  ...
  const handleRuleUpdated = (rule: PriceRuleConfig) => {
    setRules((prev) =>
  ...
  const handleRuleDelete = (rule: PriceRuleConfig) => {
  ...
  const handleSaveAll = async () => {
    ...
    const priceRules = rules.map((rule) => ({
      type: rule.ruleType,
    ...
    try {
      ...
      toast({
        title: 'Failed to update price rules',
            ...
            <>
              ...
              {rules.length === 0 ? (
                <div className="py-6 text-sm text-center text-muted-foreground">
                  No price rules yet. Click "Add rule" to add one.
                </div>
              ) : (
