...
export const SelectVoucherCampaignProvider = ({
  ...
  useEffect(() => {
    const normalizedIds = Array.isArray(value) ? value : value ? [value] : [];
    setSelectedVoucherCampaigns((prev) =>
      prev.filter((c) => normalizedIds.includes(c._id)),
    );
  }, [value]);
...
export const SelectVoucherCampaignFilterView = ({
  ...
  const [voucherCampaign, setVoucherCampaign] =
    useQueryState<ValueChangeValueType>(queryKey || 'voucherCampaignId');
  const { resetFilterState } = useFilterContext();

  return (
    <Filter.View filterKey={queryKey || 'voucherCampaignId'}>
      ...
      >
        <Popover open={open} onOpenChange={setOpen}>
          <Popover.Trigger asChild>
            <Filter.BarButton filterKey={queryKey || 'voucherCampaignId'}>
