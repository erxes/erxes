...
company, via a `customerType` toggle) plus a **broker** condition,
**independent of** the `appliesTo` product switch.

Both forms render one shared component,
`edit-pricing/components/options/CustomerBrokerConditions.tsx`, which also owns the
`CUSTOMER_BROKER_DEFAULTS`, `customerBrokerFromDetail`, and `customerBrokerToDoc`
helpers (load / save mapping). Host forms (`OptionsInfo`, `PricingCreateSheet`)
extend `CustomerBrokerFormValues` and delegate the section + the save slice.
...
| `brokerSegmentId` | `SelectSegment` (single → `[id]`) | `brokerSegmentIds` |
...
persisted** (`customerBrokerToDoc` clears the inactive kind), so a plan never
