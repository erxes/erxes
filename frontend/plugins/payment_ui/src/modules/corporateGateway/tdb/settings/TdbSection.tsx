...
import { useTdbConfigs } from './useTdbConfigs';
import { tdbColumns } from './TdbColumns';
...
const TdbRow = ({ addConfig, editConfig, original, ...props }: RowProps) => {
...
export const TdbSection = ({ searchValue }: { searchValue: string }) => {
  ...
  const CustomRow = useCallback(
    (props: any) => (
      <TdbRow {...props} addConfig={addConfig} editConfig={editConfig} />
    ),
      ...
      <Collapsible defaultOpen>
        <Collapsible.Trigger asChild>
          <div className="group flex items-center justify-between border-b pr-3 h-14 rounded-t-md cursor-pointer hover:bg-accent-foreground/10 transition-colors">
            <div className="flex items-center gap-3 px-3 flex-1 min-w-0">
              ...
              <img
                src={`${REACT_APP_API_URL}/pl:payment/static/images/payments/tdb.png`}
                alt={TDB_PAYMENT.name}
                className="h-6 w-6 rounded-md object-contain"
              />
              <div className="flex flex-col items-start">
                <span className="font-medium text-sm">{TDB_PAYMENT.name}</span>
        ...
        <Collapsible.Content>
          <RecordTable.Provider
            ...
            columns={tdbColumns({ addConfig, editConfig, removeConfig })}
            tableId="corporate-gateway-tdb-v2"
          ...
          >
            ...
            <GatewaySelectionSync bankKey="tdb" removeConfig={removeConfig} />
          </RecordTable.Provider>
        </Collapsible.Content>
      </Collapsible>
    </div>
