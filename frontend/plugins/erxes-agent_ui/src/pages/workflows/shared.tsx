import { Badge, formatDateISOStringToRelativeDate } from 'erxes-ui';
import { IWorkflowDefinition } from './types';
...
export const triggerLabel = (definition?: IWorkflowDefinition): string => {
...
export const stepCount = (definition?: IWorkflowDefinition): number =>
  Array.isArray(definition?.steps) ? definition.steps.length : 0;

/** Elapsed wall-clock time of a run (ms→s→m precision). Bespoke: the shared
 *  relative-date util compares against "now", not two arbitrary timestamps. */
...
/** Relative ("3 hours ago") label for an ISO timestamp, em dash when absent. */
export const formatTimestamp = (isoDate?: string): string =>
  isoDate ? formatDateISOStringToRelativeDate(isoDate) : '—';
