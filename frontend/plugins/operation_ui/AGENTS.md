...
- Task and project detail right rails expose configured custom properties in an editable Properties panel; the empty state is centered within the available rail height.
...
| Property side panel  | `frontend/plugins/operation_ui/src/modules/operation/components/PropertiesSidePanel.tsx` | Renders settings-configured task and project fields in the detail right rail and centers the empty state. |
...
- The property side panel removes the shared form's outer card inset so property groups fit the narrow rail, while its scroll viewport fills the rail height to center the empty state.
