...
const BulkDelete = ({
  ...
  rows: Row<TStageInErkhetConfigRow>[];
  onDeleteMany: (ids: string[]) => Promise<void>;
  loading: boolean;
}) => {
...
export const StageInErkhetConfigCommandBar = ({ onDeleteMany, loading }: Props) => {
  const { table } = RecordTable.useRecordTable();
  const selectedRows = table.getFilteredSelectedRowModel().rows as Row<TStageInErkhetConfigRow>[];
