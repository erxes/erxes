...
import { useAddTicketStatus } from '@/settings/hooks/useAddTicketStatus';
...
import { useGetTicketStatusByType } from '@/settings/hooks/useGetTicketStatusByType';
// import { useUpdateStatus } from '@/settings/hooks/useUpdateStatus';
...
export const StatusGroup = ({ statusType }: { statusType: number }) => {
  const { statuses = [], loading } = ({ type: statusType });
  const { updateStatus } = useUpdateStatus();
  const { toast } = useToast();
