...
import {
  Bar, BarChart, Area, AreaChart, CartesianGrid, Cell, Legend,
  Pie, PieChart, PolarAngleAxis, PolarGrid, PolarRadiusAxis,
  Radar, RadarChart, Tooltip, XAxis, YAxis,
} from 'recharts';
...
export const TicketSource = ({
  ...
  if (loading) {
    return (
      <FrontlineCard id={id} title={title} description="Tickets by source" colSpan={colSpan} onColSpanChange={onColSpanChange}>
  ...
  if (!allSources.length) {
    return (
      <FrontlineCard id={id} title={title} description="No ticket sources found." colSpan={colSpan} onColSpanChange={onColSpanChange}>
...
export const TicketSourceBarChart = memo(function TicketSourceBarChart({ sources }: { sources: SourceData[] }) {
  const chartConfig = useMemo(() => ({ count: { label: 'Count', color: 'var(--primary)' } }), []);
  const chartData = useMemo(() => sources.map((s) => ({ source: s.name || s._id || 'Unknown', count: s.count || 0 })), [sources]);
...
export const TicketSourceLineChart = memo(function TicketSourceLineChart({ sources }: { sources: SourceData[] }) {
  const chartConfig = useMemo(() => ({
    count: { label: 'Count', color: 'var(--primary)' },
    percentage: { label: 'Percentage', color: 'var(--success)' },
  }), []);
  const chartData = useMemo(() => sources.map((s) => ({ source: s.name || s._id || 'Unknown', count: s.count || 0, percentage: s.percentage || 0 })), [sources]);
  return (
    <ChartContainer config={chartConfig} className="aspect-video w-full">
      <AreaChart data={chartData} margin={{ top: 10 }}>
        <CartesianGrid vertical={false} strokeDasharray="3 3" />
        <XAxis dataKey="source" tickLine={false} axisLine={false} />
        <YAxis yAxisId="count" tickLine={false} axisLine={false} label={{ value: 'Count', angle: -90, position: 'insideLeft' }} />
        <YAxis yAxisId="percentage" orientation="right" tickLine={false} axisLine={false} label={{ value: 'Percentage (%)', angle: 90, position: 'insideRight' }} />
        <Area yAxisId="count" dataKey="count" type="monotone" stroke="var(--primary)" fill="var(--primary)" fillOpacity={0.3} strokeWidth={2} strokeLinecap="round" />
        <Area yAxisId="percentage" dataKey="percentage" type="monotone" stroke="var(--success)" fill="var(--success)" fillOpacity={0.3} strokeWidth={2} strokeLinecap="round" />
        <Legend content={(props: any) => <CustomLegendContent {...props} />} />
        <Tooltip content={<ChartTooltipContent />} />
      </AreaChart>
    </ChartContainer>
...
export const TicketSourcePieChart = memo(function TicketSourcePieChart({ sources }: { sources: SourceData[] }) {
  const [hovered, setHovered] = useState<string | undefined>(undefined);
  const chartConfig = useMemo(() => ({ count: { label: 'Count', color: 'var(--primary)' } }), []);
  const chartData = useMemo(() => sources.map((s, i) => ({ source: s.name || s._id || 'Unknown', count: s.count || 0, fill: CHART_COLORS[i % CHART_COLORS.length] })), [sources]);
  return (
    <ChartContainer config={chartConfig} className="aspect-video w-full">
      <PieChart>
        <Pie dataKey="count" data={chartData} cx="50%" cy="50%" outerRadius={100} innerRadius={70} nameKey="source">
          {chartData.map((item, i) => (
            <Cell key={i} fill={item.fill} opacity={hovered && hovered !== item.source ? 0.5 : 1} />
          ))}
        </Pie>
        <Legend content={(props: any) => <CustomLegendContent {...props} onMouseEnter={(d: LegendPayload) => setHovered(d.value as string)} onMouseLeave={() => setHovered(undefined)} />} />
        <Tooltip content={<ChartTooltipContent />} />
      </PieChart>
    </ChartContainer>
  );
...
export const TicketSourceRadarChart = memo(function TicketSourceRadarChart({ sources }: { sources: SourceData[] }) {
  const chartConfig = useMemo(() => ({
    count: { label: 'Count', color: 'var(--primary)' },
    percentage: { label: 'Percentage', color: 'var(--success)' },
  }), []);
  const maxCount = useMemo(() => Math.ceil(Math.max(...sources.map((s) => s.count || 0), 0) / 10) * 10 || 100, [sources]);
  const maxPercentage = useMemo(() => Math.max(...sources.map((s) => s.percentage || 0), 100), [sources]);
  const scaleFactor = maxCount > 0 && maxPercentage > 0 ? maxCount / maxPercentage : 1;
  const chartData = useMemo(() => sources.map((s) => ({
  ...
  return (
    <ChartContainer config={chartConfig} className="aspect-video w-full">
      <RadarChart data={chartData}>
        ...
        <Radar name="Percentage" dataKey="percentage" stroke="var(--success)" fill="var(--success)" fillOpacity={0.3} />
        <Legend content={(props: any) => <CustomLegendContent {...props} />} />
        <Tooltip content={<ChartTooltipContent />} formatter={(v: number, name: string, props: any) => name === 'Percentage' ? [props.payload.percentageOriginal?.toFixed(1) + '%', 'Percentage'] : [v, name]} />
      </RadarChart>
    </ChartContainer>
  );
});

export const TicketSourceTableChart = memo(function TicketSourceTableChart({ sources }: { sources: SourceData[] }) {
  const columns: ColumnDef<SourceData>[] = [
    { id: '_id', header: 'Source', accessorKey: '_id', cell: ({ cell }) => <RecordTableInlineCell className="px-4 text-xs">{cell.getValue() as string}</RecordTableInlineCell>, size: 28 },
    { id: 'name', header: 'Name', accessorKey: 'name', cell: ({ cell }) => <RecordTableInlineCell className="px-4 text-xs truncate">{cell.getValue() as string}</RecordTableInlineCell> },
    {
      id: 'count', header: 'Data', accessorKey: 'count', size: 28,
      cell: ({ cell }) => {
        const { count, percentage } = cell.row.original || {};
        return <RecordTableInlineCell className="px-3 text-xs flex items-center justify-end text-muted-foreground">{count} / {percentage}%</RecordTableInlineCell>;
      },
    },
  ];
  return (
