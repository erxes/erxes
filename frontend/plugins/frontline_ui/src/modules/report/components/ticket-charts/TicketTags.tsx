...
export const TicketTags = ({
  ...
  if (loading) {
    return (
      <FrontlineCard id={id} title={title} description="Tickets by tag" colSpan={colSpan} onColSpanChange={onColSpanChange}>
  ...
  if (!allTags.length) {
    return (
      <FrontlineCard id={id} title={title} description="No ticket tags found." colSpan={colSpan} onColSpanChange={onColSpanChange}>
...
export const TicketTagBarChart = memo(function TicketTagBarChart({ tags }: { tags: TagData[] }) {
  ...
  const chartData = useMemo(() => tags.map((t) => ({
    tag: t.name || 'Unknown',
    count: t.count || 0,
  ...
  return (
    <ChartContainer config={chartConfig} className="aspect-video w-full">
      <BarChart data={chartData}>
        ...
        <Tooltip content={<ChartTooltipContent />} formatter={(v: number, name: string, props: any) => name === 'Percentage' ? [props.payload.percentage?.toFixed(1) + '%', 'Percentage'] : [v, name]} />
      </BarChart>
    </ChartContainer>
  );
});
...
export const TicketTagLineChart = memo(function TicketTagLineChart({ tags }: { tags: TagData[] }) {
  ...
  const chartData = useMemo(() => tags.map((t) => ({ tag: t.name || 'Unknown', count: t.count || 0, percentage: t.percentage || 0 })), [tags]);
  return (
    <ChartContainer config={chartConfig} className="aspect-video w-full">
...
export const TicketTagPieChart = memo(function TicketTagPieChart({ tags }: { tags: TagData[] }) {
  ...
  const chartData = useMemo(() => tags.map((t, i) => ({ tag: t.name || 'Unknown', count: t.count || 0, fill: CHART_COLORS[i % CHART_COLORS.length] })), [tags]);
  return (
    <ChartContainer config={chartConfig} className="aspect-video w-full">
      <PieChart>
        <Pie dataKey="count" data={chartData} cx="50%" cy="50%" outerRadius={100} innerRadius={70} nameKey="tag">
...
export const TicketTagRadarChart = memo(function TicketTagRadarChart({ tags }: { tags: TagData[] }) {
  const chartConfig = useMemo(() => ({
    count: { label: 'Count', color: 'var(--primary)' },
    percentage: { label: 'Percentage', color: 'var(--success)' },
  }), []);
  const maxCount = useMemo(() => Math.ceil(Math.max(...tags.map((t) => t.count || 0), 0) / 10) * 10 || 100, [tags]);
  const maxPct = useMemo(() => Math.max(...tags.map((t) => t.percentage || 0), 100), [tags]);
  const scaleFactor = maxCount > 0 && maxPct > 0 ? maxCount / maxPct : 1;
  const chartData = useMemo(() => tags.map((t) => ({
    tag: t.name || 'Unknown',
...
export const TicketTagTableChart = memo(function TicketTagTableChart({ tags }: { tags: TagData[] }) {
  const columns: ColumnDef<TagData>[] = [
    { id: 'name', header: 'Tag', accessorKey: 'name', cell: ({ cell }) => <RecordTableInlineCell className="px-4 text-xs">{cell.getValue() as string}</RecordTableInlineCell> },
    {
