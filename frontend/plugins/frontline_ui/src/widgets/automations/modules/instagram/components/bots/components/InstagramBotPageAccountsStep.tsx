      ...
      <Command>
        ...
        <div className="flex justify-between items-center px-1 py-2">
          ...
          <Button variant="ghost" className="text-pink-600" asChild>
            <a
              href={`https://90uv5wilprnx.share.zrok.io/pl:frontline/instagram/iglogin?kind=${IntegrationType.INSTAGRAM_MESSENGER}`}
              target="_blank"
              rel="noreferrer"
            >
              <IconPlus />
              {t('add-account-via-instagram', 'Add account via Instagram')}
            </a>
          </Button>
        </div>
        <RadioGroup
  ...
  return instagramGetAccounts.map((account) => (
    <Command.Item
      ...
      className={cn(
        ...
        selectedAccountId === account._id && 'text-pink-600',
      )}
    >
