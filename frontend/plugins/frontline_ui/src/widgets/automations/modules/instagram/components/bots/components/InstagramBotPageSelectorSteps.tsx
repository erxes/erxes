...
import { InstagramBotPageAccountsStep } from '~/widgets/automations/modules/instagram/components/bots/components/InstagramBotPageAccountsStep';
import { InstagramBotPagesStep } from '~/widgets/automations/modules/instagram/components/bots/components/InstagramBotPagesStep';
...
const useActionButton = (accountId?: string) => {
  ...
  const selectedAccountId =
    useAtomValue(selectedInstagramAccountAtom) || accountId;
  const selectedPageId = useAtomValue(selectedInstagramPageAtom);
...
export const InstagramBotPageSelectorSteps = ({
  step,
  accountId,
}: {
  step: number;
  ...
  return (
    <>
      ...
      <Sheet.Content>
            ...
            {Array.from({ length: 2 }).map((_, index) => (
              <div
                ...
                className={cn(
                  ...
                  activeStep === index + 1 && 'bg-pink-600',
                )}
              />
            ))}
          </div>
        ...
        {activeStep === 1 && (
          <InstagramBotPageAccountsStep accountId={accountId} />
        )}
        {activeStep === 2 && <InstagramBotPagesStep />}
      </Sheet.Content>
      <Sheet.Footer>
        <Button
          variant="secondary"
