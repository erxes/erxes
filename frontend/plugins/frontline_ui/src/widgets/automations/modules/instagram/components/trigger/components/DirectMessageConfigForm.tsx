...
const DirectMessageCondition = ({
  ...
  const onRemoveConditionKeyword = (_id: string) => {
    onChange('keywords', condition.keywords.filter((keyword) => keyword._id !== _id));
  };
    ...
    if (e.key === 'Enter') {
      onChange(
        ...
        condition.keywords.map((keyword) =>
          keyword._id === id ? { ...keyword, text: value, isEditing: false } : keyword,
        ),
      );
    }
  };
    ...
    return (
      <div className="flex flex-wrap gap-4 py-4">
        {(condition.keywords || []).map(({ _id, text, isEditing }) => (
          <Badge key={_id} id={_id} onDoubleClick={toggleKeywordState} onClose={() => onRemoveConditionKeyword(_id)}>
            {isEditing ? (
              <Input
                key={_id}
                id={_id}
...
export const DirectMessageConfigForm = ({ conditions = [], onConditionChange }: Props) => {
  const onRemoveCondition = (id: string) => {
    onConditionChange('conditions', conditions.filter((cond) => cond._id !== id));
  };
  ...
  return (
    <div>
      <div className="flex justify-end">
        <Button variant="ghost" onClick={onAddCondition}>+ Add condition</Button>
      </div>
      {conditions.map((condition) => {
        ...
        return (
          <div className="border rounded-md p-4 relative group mt-4 hover:shadow" key={condition._id}>
            <Button
              variant="destructive"
              size="icon"
