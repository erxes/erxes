...
const getRuleLabel = (rule?: AccountingDealRule) =>
  rule?.value?.title || rule?.subId || rule?._id || 'Select config';
...
const useApplyDealRuleFilter = () => {
  const [{ ruleId }, setQueries] = useMultiQueryState<{
    ruleId: string;
    boardId: string;
    pipelineId: string;
    stageId: string;
  ...
  return {
    ...
    applyRule: (rule?: AccountingDealRule) => {
      setQueries({
        ...
        boardId: rule?.value?.boardId || null,
        pipelineId: rule?.value?.pipelineId || null,
        stageId: rule?.value?.stageId || rule?.subId || null,
      });
    },
  };
};
...
const AccountingCheckSyncedDealRuleContent = ({
  ...
  return (
    <Command>
      ...
      <Command.Empty>
        <span className="text-muted-foreground">No configs found</span>
      </Command.Empty>
      <Command.List>
        {rules.map((rule) => (
          <Command.Item
