...
const renderTableCell = (cell: EmailTableCell, isHeader: boolean) => {
  ...
  return `<${tag}${colspan}${rowspan} style="${styles.join(
    '; ',
  )}">${renderInlineContent(content)}</${tag}>`;
};
...
/**
 * Converts BlockNote editor blocks into email-safe HTML (inline styles only).
 ...
 */
export const renderEmailBlocks = (blocks: readonly EmailEditorBlock[]) => {
  const html: string[] = [];
  ...
  for (const block of blocks) {
    ...
    flushList();
    html.push(renderEmailBlock(block));
  }
