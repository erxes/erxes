  ...
  if (ui_location) {
    ...
    if (!tag) {
      ...
      if (tag === 'dev') {
        ...
        s3_location = `https://erxes-timeclock-plugin.s3.us-west-2.amazonaws.com/uis/${plName}`;
      }
    }
...
const up = async ({ uis, fromInstaller }) => {
  ...
  if (configs.image_tag) {
    if (configs.image_tag === 'dev') {
      ...
      pluginsMapLocation = `https://erxes-timeclock-plugin.s3.us-west-2.amazonaws.com/timeclock/pluginsMap.js`;
    }
  }
