...
export default async function build(folderName: string) {
    ...
    fs.cpSync(`${erxesDir}/packages/api-utils`, `${artifactDir}/packages/api-utils`, { recursive: true });
    fs.cpSync(`${erxesDir}/packages/tsconfig.api.jsonc`, `${artifactDir}/packages/tsconfig.api.jsonc`);
    fs.cpSync(`${erxesDir}/packages/${folderName}`, `${artifactDir}/packages/${folderName}`, { recursive: true });
    ...
    execSync(`cp -RT ${artifactDir}/packages/api-utils/dist ${artifactDir}/packages/api-utils/src`);
    execSync(`cp -RT ${artifactDir}/packages/${folderName}/dist ${artifactDir}/packages/${folderName}/src`);
    ...
    if(fs.existsSync(`${artifactDir}/packages/${folderName}/Dockerfile`)) {
      execSync(`cp ${erxesDir}/packages/${folderName}/Dockerfile ${artifactDir}/Dockerfile`);
    } else // provide default Dockerfile
    {
      const dockerfileTemplate = fs.readFileSync(`${erxesDir}/packages/default.template.Dockerfile`).toString();
