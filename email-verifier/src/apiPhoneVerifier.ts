...
const phoneValidUrl = 'https://api.clearoutphone.io/v1';
...
const singleClearOut = async (phone: string): Promise<any> => {
  try {
    const response = await sendRequest({
      url: `${phoneValidUrl}/phonenumber/validate`,
      method: 'POST',
      headers: {
...
export const sendFile = async (fileName: string, hostname: string) => {
  try {
    const result = await request({
      method: 'POST',
      url: `${phoneValidUrl}/phonenumber/bulk`,
      headers: {
...
export const getStatus = async (listId: string) => {
  const url = `${phoneValidUrl}/phonenumber/bulk/progress_status?list_id=${listId}`;
  try {
    const result = await request({
      method: 'GET',
      url,
...
export const getBulkResult = async (listId: string, hostname: string) => {
  const url = `${phoneValidUrl}/download/result`;
  const headers = {
    'Content-Type': 'application/json',
    Authorization: `Bearer:${CLEAR_OUT_PHONE_API_KEY}`
  };
