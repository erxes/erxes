...
module.exports.up = async () => {
  ...
  try {
    const integrations = await mongoClient.db
      ...
      .find({ kind: { $eq: 'facebook' }, facebookPageTokensMap: { $exists: false } })
      .toArray();

    for (const integration of integrations) {
      const facebookPageTokensMap = {};
      const account = await mongoClient.db.collection('accounts').findOne({ _id: integration.accountId });

      for (const pageId of integration.facebookPageIds) {
        facebookPageTokensMap[pageId] = await getPageAccessToken(pageId, account.token);
      }

      await mongoClient.db
        .collection('integrations')
        .updateOne({ _id: integration._id }, { $set: { facebookPageTokensMap } });
    }
  } catch (e) {
