...
describe("engage messages model tests", () => {
  ...
  test("save matched customer ids", async () => {
    const message = await EngageMessages.setCustomerIds(_message._id, [
      _customer,
      _customer2
    ]);
  ...
  test("add new delivery report", async () => {
    ...
    expect(message.deliveryReports[`${mailMessageId}`].status).toEqual(
      "pending"
    );
    expect(message.deliveryReports[`${mailMessageId}`].customerId).toEqual(
      _customer._id
    );
  });
