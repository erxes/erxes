...
describe('dashboardQueries', () => {
  ...
  test('Dashboard detail', async () => {
    ...
    const response = await graphqlRequest(qry, 'dashboardDetails', { _id: dashboard._id });
  ...
  test('Get dashboard items', async () => {
    ...
    const dashboardItems = await graphqlRequest(qry, 'dashboardItems', { dashboardId: dashboard._id });
  ...
  test('Get dashboard items', async () => {
    ...
    const dashboardItem = await dashboardItemsFactory({ name: 'dashboardItem name' });

    const dashboardItemDetail = await graphqlRequest(qry, 'dashboardItemDetail', { _id: dashboardItem._id });
